...
class reference_images(omega_scan):
    ...
    def analyze(self):
        command = '/home/experiences/proxima2a/com-proxima2a/mxcube_local/HardwareRepository/HardwareObjects/SOLEIL/PX2/experimental_methods/reference_images.py'
        sense_line = '%s -d %s -n %s -A &' % (command, self.directory, self.name_pattern)
    ...
    def rectify_master(self, timeout=15):
        ...
        #shutil.copy2('{directory:s}/{name_pattern:s}_master.h5'.format(directory=self.directory, name_pattern=self.name_pattern), '{directory:s}/{name_pattern:s}_master.h5_0001'.format(directory=self.directory, name_pattern=self.name_pattern))
                      
        m = h5py.File('%s/%s_master.h5' % (self.directory, self.name_pattern))
    ...
    def generate_summed_h5(self):
        ...
        generate_summed_h5_line = 'cd {directory}; /home/experiences/proxima2a/com-proxima2a/mxcube_local/HardwareRepository/HardwareObjects/SOLEIL/PX2/experimental_methods/summer.py -n {nimages_per_file} -m {name_pattern}_master.h5 &'.format(**self.format_dictionary)
        os.system(generate_summed_h5_line)

    def generate_cbf(self):
        print('generate_cbf')
        generate_cbf_line = 'cd {directory}; /usr/local/bin/H5ToCBF.py -m {name_pattern}_master.h5 -d {directory}/process'.format(**self.format_dictionary)
