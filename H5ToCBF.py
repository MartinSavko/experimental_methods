...
stream_formatter = logging.Formatter(
    "H5ToCBF.py |%(asctime)s |%(levelname)-7s| %(message)s"
)
...
oscillation_axes_ranges = {
    "OMEGA": omega_range_average,
...
def get_header_information(master_file):
    ...
    h["oscillation_axis_values"] = master_file[
        "/entry/sample/goniometer/%s" % oscillation_axis.lower()
    ][()]
...
def get_nimages(master_file, first, last):
    nimages = sum(
        [d.shape[0] for d in list(master_file["/entry/data"][()]) if d is not None]
    )
...
def extract_cbfs(
    ...
    for wedge in wedges:
        ...
        wedge_end = time.time()
        log.info(
            "wedge %3d (of %d), wedge processing time %6.4f s, time per image in this wedge %6.4f s; total processing time %5.1f s, which is %6.4f s per image"
            % (
                k,
                len(wedges),
                wedge_end - wedge_start,
                (wedge_end - wedge_start) / len(wedge),
                time.time() - start,
                (time.time() - start) / nsofar,
            )
        )
    end = time.time()
    log.info(
        "total processing time %.1f s, which is %.3f s per image"
        % (end - start, (end - start) / nimages)
    )
...
def save_image(
    ...
    filename = os.path.basename(
        filename_template.replace("######", str(image_number).zfill(6))
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        default=0,
        type=int,
        help="Number of parallel extraction, by defalult it will determine the number of cores of the machine and use all of them.",
    )
    parser.add_option(
        "-f",
        "--first",
        default=0,
        type=int,
        help="First image to extract. Default is the first one.",
    )
    parser.add_option(
        "-l",
        "--last",
        default=-1,
        type=int,
        help="Last image to extract. Default is the last one.",
    )
    parser.add_option(
        "-b", "--bzip2", action="store_true", help="Compress cbf files using bzip2."
    )
    parser.add_option(
        "-g", "--gzip", action="store_true", help="Compress cbf files using gzip."
    )
    parser.add_option(
    ...
    filename_template = os.path.join(
        destination_directory,
        os.path.basename(master_file.filename.replace("_master.h5", "_?????.cbf")),
    )

    cbf_files = glob.glob(filename_template)
    if nimages == len(cbf_files) and options.overwrite != True:
        print(
            "It seems conversion of %s to cbf has already been done. Please use option --overwrite if you wish to regenerate the files. Exiting ..."
        )
        sys.exit()

    print(
        "Starting h5 to cbf conversion. The conversion time per image should be below 0.1 second. If it is more, there may be something wrong with the system."
    )
    print(
        "%d images to extract. Using %d threads. Processing in %d wedges of %d images.\n"
        % (nimages, options.n_cpu, len(wedges), len(wedges[0]))
    )

    extract_cbfs(
        master_file,
        master_file_absolute_path.replace(
            source_directory, options.treatment_directory
        ),
        destination_directory,
        options.first,
        options.last,
