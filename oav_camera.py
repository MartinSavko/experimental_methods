...
class oav_camera(zmq_camera):
    ...
    def __init__(
        ...
        print(f"codec {codec}, mode {mode},{service}")
        self.mode = mode
    ...
    def get_last_image_data(self):
        ...
        if self.mode == "redis_local":
            last_image_data = self.redis_local.get(f"last_image_data_{self.service_name}")
            ...
            img = np.reshape(raw, (self.y_pixels_in_detector, self.x_pixels_in_detector, 3))
    ...
    def get_beam_position(self):
        return np.array(self.get_shape())
...
def main():
    ...
    parser.add_argument("-s", "--service", type=str, default="oav_camera", help="debug string add to the outputs")
    parser.add_argument("-v", "--verbose", action="store_true", help="verbose")
    parser.add_argument("-o", "--codec", type=str, default="h264", help="video codec")
    args = parser.parse_args()
    print(args)
    
    cam = oav_camera(mode=args.mode, service=args.service, debug_frequency=args.debug_frequency, codec=args.codec, verbose=False)
