...
def main(results_filename='s3b_results.pickle'):
    ...
    s = pickle.load(open(results_filename))
    ...
    if 's2' in results_filename or 's1' in results_filename:
        if 's1' in results_filename:
            labels = ['i11-ma-c02/ex/fent_h.1-mt_i', 'i11-ma-c02/ex/fent_h.1-mt_o',
                      'i11-ma-c02/ex/fent_v.1-mt_u', 'i11-ma-c02/ex/fent_v.1-mt_d']
            tex_labels = {'i11-ma-c02/ex/fent_h.1-mt_i': 'horizontal in', 
            ...
            labels = ['i11-ma-c04/ex/fent_h.2-mt_i', 'i11-ma-c04/ex/fent_h.2-mt_o',
                      'i11-ma-c04/ex/fent_v.2-mt_u', 'i11-ma-c04/ex/fent_v.2-mt_d']
            tex_labels = {'i11-ma-c04/ex/fent_h.2-mt_i': 'horizontal in', 
        ...
        for s, label in zip([hi, ho, vu, vd], labels):
            ...
            ip = interp1d(position, transmission, fill_value=tuple(fill_value), bounds_error=False, kind='slinear')
            ...
            pylab.plot(extended_positions, calculated_transmission, label='%s' % tex_labels[label])       
            ...
            pylab.savefig('%s/slit_scan_%s_%s.png' % (os.path.dirname(results_filename), os.path.basename(results_filename).replace('_results.pickle', ''), tex_labels[label].replace(' ', '_')))
            
            contribution = blank_slate * np.abs(np.gradient(ndi.gaussian_filter1d(calculated_transmission, 7)))
        ...
        if 's3' in results_filename:
            ...
            tex_labels = {'i11-ma-c05/ex/fent_v.3-mt_tz': 'vertical', 
            ...
            tex_labels = {'i11-ma-c06/ex/fent_v.5-mt_tz': 'vertical', 
            ...
            tex_labels = {'i11-ma-c06/ex/fent_v.6-mt_tz': 'vertical', 
    ...
    np.save('%s/distribution_%s.npy' % (os.path.dirname(results_filename), os.path.basename(results_filename).replace('_results.pickle', '')) , distribution)
    start_ii = time.time()
    ii = skimage.transform.integral.integral_image(distribution)
    end_ii = time.time()
    print('integral image generated in %.3f seconds' % (end_ii - start_ii))
    
    np.save('%s/ii_%s.npy' % (os.path.dirname(results_filename), os.path.basename(results_filename).replace('_results.pickle', '')) , ii)
    
    def get_indices_for_slit_setting(horizontal_gap, vertical_gap, horizontal_center=0., vertical_center=0., npixels=4000, extent=(-2, 2)):
        e = extent[1] - extent[0]
        ...
        return (int(vertical_start), int(horizontal_start)), (int(vertical_end), int(horizontal_end))
        
    start, end = get_indices_for_slit_setting(1., 1.)
    ...
    _start = time.time()
    print('opening 1., 1. from sum', distribution[start[0]:end[0]+1,start[1]:end[1]+1].sum())
    _end = time.time()
    print('single sum took %.3f' % (_end - _start))
    
    start, end = get_indices_for_slit_setting(1., 1., -0.5, -0.5)
    print('start, end', start, end)
    print('opening 1., 1.', skimage.transform.integral.integrate(ii, start, end))
    ...
    start, end = get_indices_for_slit_setting(1., 1., -1, -1)
    print('start, end', start, end)
    print('opening 1., 1.', skimage.transform.integral.integrate(ii, start, end))
    ...
    pylab.savefig('%s/photon_distribution_%s_2d.png' % (os.path.dirname(results_filename), os.path.basename(results_filename).replace('_results.pickle', '')))
    ...
    #pylab.savefig('photon_distribution_primary_slits_2d.png')
    pylab.savefig('%s/photon_distribution_%s_integral_image.png' % (os.path.dirname(results_filename), os.path.basename(results_filename).replace('_results.pickle', '')))
    ...
    ax.set_title('Photon distribution from slit scans', fontsize=24)
    pylab.savefig('%s/photon_distribution_%s_3d_auto.png' % (os.path.dirname(results_filename), os.path.basename(results_filename).replace('_results.pickle', '')))

if __name__ == '__main__':
    import optparse
    parser = optparse.OptionParser()
    parser.add_option('-r', '--results_filename', default='s1b_results.pickle', type=str, help='results file name')
    parser.add_option('-D', '--display', action='store_true', help='display graphs')
    options, args = parser.parse_args()
    
    main(results_filename=options.results_filename)
    if options.display:
        pylab.show()