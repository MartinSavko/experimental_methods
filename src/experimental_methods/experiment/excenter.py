...
def main():
    ...
    parser.add_option(
        "-d",
        "--directory",
        default="/nfs/ruche/proxima2a-spool/2019_Run1/x-centring",
        type=str,
    ...
    parser.add_option(
        ...
        default="excenter",
        type=str,
        help="Name pattern (default: %default)",
    )
    parser.add_option(
        "-c",
        "--calculate",
        action="store_true",
        help="Just calculate position, do not actually move the motors",
    )
    parser.add_option(
        "-a",
        "--angles",
        default="(90, 0, 270)",
        type=str,
        help="Specify angles for grid scans that will be used for x-centring",
    )
    parser.add_option(
        "-l",
        "--length",
        default=0.3,
        type=float,
        help="Specify the length of scanned area in mm (default: %default) ",
    )
    parser.add_option(
        "-w",
        "--width",
        default=0.01,
        type=float,
        help="Specify the widht of scanned area in mm (default: %default)",
    )
    parser.add_option(
        "-i",
        "--interpret",
        action="store_true",
        help="Just analyse the results from images already taken (do not collect anew).",
    )
    parser.add_option(
        "-s",
        "--step_size",
        default=0.002,
        type=float,
        help="step_size in mm (default: %default)",
    )
...
def execute_grid_scans(
    orientations, directory, name_pattern, length=0.1, step_size=0.002
):
    ...
    for orientation in orientations:
        if not os.path.exists(
            os.path.join(directory, "%s_%s_master.h5" % (name_pattern, orientation))
        ):
            gonio.set_position({"Omega": orientation}, wait=True)
            os.system(
                "area_scan.py -d %s -p %s -n %s_%s -y %s -x 0.01 -c 1 -r %s -a vertical"
                % (
                    directory,
                    orientation,
                    name_pattern,
            ...
            print(
                "%s already exists, skipping ...\nPlease change the destination directory or the name_pattern."
                % os.path.join(
                    directory, "%s_%s_master.h5" % (name_pattern, orientation)
                )
            )
...
def get_move_vector_mm(move_vector, calibration):
    d_sampx, d_sampy, d_y, d_z = move_vector
    pixels_per_mm_y = 1.0 / calibration[1]
    pixels_per_mm_z = 1.0 / calibration[0]
...
def get_beam_center(parameters):
    beam_xc, beam_yc = (
        parameters["beam_position_horizontal"],
        parameters["beam_position_vertical"],
    )
...
def get_calibration(parameters):
    return (
        parameters["camera_calibration_vertical"],
        parameters["camera_calibration_horizontal"],
    )
...
def get_imagenames(directory, template, n_angles=3, wait_cycles=40):
    ...
    while len(imagenames) < n_angles and k < wait_cycles:
        ...
        imagenames = glob.glob(os.path.join(directory, template))
        print(
            "waiting for analysis to finish ... timeout in %s seconds"
            % (wait_cycles - k)
        )
    return imagenames
...
def get_angles(imagenames, template):
    angles = [
        float(re.findall(template.replace("*", "([-|+]?[\d\.]*)"), imagename)[0])
