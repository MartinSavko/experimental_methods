...
def residual(x, data_matrix):
    ...
    experiment = data[:, 2]
    return 1.0 / (2 * len(theory)) * np.sum((experiment - theory) ** 2)


def residual2(x, gaps, bs):
    k0, k1, k2 = x
    theory = undulator_magnetic_field(gaps, k0, k1, k2)
    return 1.0 / 2 * (len(theory)) * np.sum((bs - theory) ** 2)
...
def undulator_peak_energy(
    gap, n, k0=2.72142696, k1=-3.83321034, k2=0.6322541, N=80, detune=False
):
    if detune:
        detune_parameter = 1 - 1 / (n * N)
    else:
...
def undulator_magnetic_field(
    gap, k0=2.72142696, k1=-3.83321034, k2=0.6322541, period_length=24.0
):
    x = gap / period_length
...
def undulator_peak_intensity(
    gap, n, k0=2.72142696, k1=-3.83321034, k2=0.6322541, period_length=24.0, N=80
):
    k = 1e-3 * elementary_charge / (electron_mass * speed_of_light * 2 * pi)
...
def get_experimental_peaks(theory, exp_energy, exp_flux, gap, data):
    ...
    for harmonic, n in theory[::3]:
        if harmonic > 4000.0 and harmonic < 20001:  # .max():
            # print('harmonic')
            # print(harmonic)
            ...
            energies = (
                me  # exp_energy[np.where(np.abs(exp_energy - maximum_energy) < 75)]
            )
...
def plot(data_matrix):
    ...
    for n in harmonics:
        ...
        ens = energies
        X = np.vstack([ens / 1.0e3, gaps]).T
        np.savetxt(
            "GAP_ENERGY_HARMONIC%s.txt" % n,
            X,
            fmt="%6.3f",
            delimiter=" ",
        ...
        np.savetxt(
            "fit_GAP_ENERGY_HARMONIC%s.txt" % n,
            X_model,
            fmt="%6.3f",
            delimiter=" ",
        ...
        pylab.plot(
            ...
            gaps,
            "o-",
            color=sns.xkcd_rgb[xkcd_colors_that_i_like[n - min(harmonics)]],
            label="%d" % n,
        )
        pylab.plot(
            modeled_energies,
    ...
    pylab.title("Proxima 2A U24 undulator tuning curves", fontsize=22)
    pylab.xlabel("energy [eV]", fontsize=18)
    ...
    pylab.title("Proxima 2A U24 undulator tuning curves, odd harmonics", fontsize=22)
    pylab.xlabel("energy [eV]", fontsize=18)
    pylab.ylabel("flux [ph/s]", fontsize=18)
    # pylab.ylim([7., 12.5])
    pylab.grid(True)
    ...
    pylab.title("Proxima 2A U24 undulator tuning curves, even harmonics", fontsize=22)
    pylab.xlabel("energy [eV]", fontsize=18)
    pylab.ylabel("flux [ph/s]", fontsize=18)
    # pylab.ylim([7., 12.5])
    pylab.grid(True)
    ...
    ax.text(
        0.05,
        0.08,
        "fit parameters: k_{0} = %6.3f, k_{1} = %6.3f, k_{2} = %6.3f" % (k0, k1, k2),
        fontsize=20,
        color="green",
        transform=ax.transAxes,
    )
...
def main():
    ...
    parser.add_option(
        "-d",
        "--data_matrix",
        default="data_0.1x0.1mm_450mA.pkl",
        type=str,
        help="Data matrix file",
    )
    parser.add_option(
        "-f",
        "--fit",
        action="store_true",
        help="Perform a fit and print out the fitted parameters",
    )
    parser.add_option(
        "-p",
        "--plot",
        action="store_true",
        help="Show the results and generate figures",
    )
