...
def get_params(
    datafile="/nfs/data2/Martin/Research/undulator/xray1018.dat",
):  # xray5184.dat /927bis/ccd/gitRepos/flux/xray9507_Si_125um.dat
...
def get_theta_from_wavelength(
    wavelength,
    units_theta=degree,
    units_energy=kilo * eV,
    units_wavelength=angstrom,
    d=3.1347507142511746,
):
    theta = (
        np.arcsin((angstrom / units_wavelength) * wavelength / (2 * d)) / units_theta
    )
    return theta


def get_wavelength_from_theta(
    theta,
    units_theta=degree,
    units_energy=kilo * eV,
    units_wavelength=angstrom,
...
def get_energy_from_theta(
    theta, units_theta=degree, units_energy=kilo * eV, units_wavelength=angstrom
):
    wavelength = get_wavelength_from_theta(
        theta, units_wavelength=units_wavelength, units_theta=units_theta
    )
    energy = get_energy_from_wavelength(
        wavelength, units_energy=units_energy, units_wavelength=units_wavelength
    )
    return energy


def get_theta_from_energy(
    energy, units_theta=degree, units_energy=kilo * eV, units_wavelength=angstrom
):
    wavelength = get_wavelength_from_energy(
        energy, units_energy=units_energy, units_wavelength=units_wavelength
    )
    theta = get_theta_from_wavelength(
        wavelength,
        units_theta=units_theta,
        units_energy=units_energy,
        units_wavelength=units_wavelength,
    )
    return theta


def get_wavelength_from_energy(
    energy, units_energy=kilo * eV, units_wavelength=angstrom
):
    wavelength = h * c / (units_wavelength * units_energy) / energy
    return wavelength


def get_energy_from_wavelength(
    wavelength, units_energy=kilo * eV, units_wavelength=angstrom
):
...
def F(K, n):
    chi = n / (1.0 + 0.5 * K**2)
    Y = 0.25 * (K**2) * chi
    return (chi**2) * (K**2) * (jv((n + 1) / 2.0, Y) - jv((n - 1) / 2.0, Y)) ** 2
...
def get_lambda_harmonic(lambda_peak, n, N=80, detune=False):
    ...
    if detune:
        detune_parameter = 1 - 1.0 / (n * N)

    return lambda_peak * detune_parameter


def undulator_peak_energy(
    gap, n, k0=2.72898056, k1=-3.83864548, k2=0.60969562, N=80, detune=False
):
    if detune:
        detune_parameter = 1 - 2.0 / (n * N)
    else:
        detune_parameter = 1
    return undulator_harmonic_energy(gap, n, k0=k0, k1=k1, k2=k2) * detune_parameter


def undulator_magnetic_field(
    gap, k0=2.72898056, k1=-3.83864548, k2=0.60969562, period_length=24.0
):
    x = gap / period_length
    return k0 * exp(k1 * x + k2 * x**2)


def undulator_strength(B, period_length=24.0):
    k = 1e-3 * elementary_charge / (electron_mass * speed_of_light * 2 * pi)
    return k * period_length * B
    # return 0.0934 * period_length * B


def undulator_magnetic_field_from_K(K, period_length=24.0):
    k = 1e-3 * elementary_charge / (electron_mass * speed_of_light * 2 * pi)
    return K / (k * period_length)


def undulator_strength_from_peak_position(
    peak_energy, n, electron_energy=2.75, period_length=24.0
):
    # return 9.5 * n * electron_energy ** 2 / ((1 +  K ** 2 / 2.) * period_length)
    return sqrt(2 * 9.5 * n * electron_energy**2 / (period_length * peak_energy) - 2)


def undulator_harmonic_energy(
    gap,
    n,
    k0=2.72898056,
    k1=-3.83864548,
    k2=0.60969562,
    period_length=24.0,
    electron_energy=2.75,
):
    k = 1e-3 * elementary_charge / (electron_mass * speed_of_light * 2 * pi)
    B = undulator_magnetic_field(gap, k0=k0, k1=k1, k2=k2, period_length=period_length)
    return (
        1000
        * 9.5
        * n
        * electron_energy**2
        / (period_length * (1 + undulator_strength(B) ** 2 / 2))
    )


def undulator_peak_intensity(
    gap, n, k0=2.72898056, k1=-3.83864548, k2=0.60969562, period_length=24.0, N=80
):
    k = 1e-3 * elementary_charge / (electron_mass * speed_of_light * 2 * pi)
...
def main():
    ...
    templates = [
        os.path.basename(filename).replace("_results.pickle", "") for filename in files
    ]
    data = []
    for template in templates:
        ...
        parameters = pickle.load(
            open(os.path.join(directory, "%s_parameters.pickle" % template), "rb"),
            encoding="bytes",
        )
        results = pickle.load(
            open(os.path.join(directory, "%s_results.pickle" % template), "rb"),
            encoding="bytes",
        )
        # print('parameters')
        # print(parameters)
        ...
        dark_current = diode_current[
            diode_chronos < start_chronos - fast_shutter_chronos_uncertainty
        ].mean()
        diode_current -= dark_current

        actuator_scan_indices = np.logical_and(
            actuator_chronos > start_chronos + fast_shutter_chronos_uncertainty * 5,
            actuator_chronos < end_chronos - fast_shutter_chronos_uncertainty * 5,
        )
        ...
        diode_scan_indices = np.logical_and(
            diode_chronos > start_chronos + fast_shutter_chronos_uncertainty * 5,
        ...
        pylab.legend()
        pylab.title(
            "Energy scan, %s mm, undulator U24 Proxima 2A, SOLEIL"
            % template.replace("_", " ")
        )
