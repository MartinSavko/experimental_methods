...
class tomography(xray_experiment):
    ...
    def get_nimages(self):
        ...
        if (
            abs(nimages * self.angle_per_frame - self.scan_range)
            > self.angle_per_frame / 0.5
        ):
            nimages += 1
        return nimages
        ...
        while self.goniometer.is_task_running(task_id):
            ...
            if new_image_id != last_image:
                ...
                self.background.append(
                    self.get_point(new_image_id, self.background_start_time)
                )
    ...
    def run(self):
        ...
        task_id = self.goniometer.omega_scan(
            self.scan_start_angle, self.scan_range, self.scan_exposure_time, wait=False
        )
    ...
    def save_results(self):
        f = open(
            os.path.join(self.directory, "%s_tomography.pickle" % self.name_pattern),
            "wb",
        )
        pickle.dump(
            {"observations": self.observations, "background": self.background}, f
        )
        f.close()
    ...
    def save_parameters(self):
        ...
        self.parameters[
            "detector_tz"
        ] = self.detector.stage_vertical_motor.get_position()
        self.parameters[
            "detector_tx"
        ] = self.detector.stage_horizontal_motor.get_position()
        self.parameters["focus"] = self.detector.focus_motor.get_position()
        self.parameters[
            "camera_vertical_motor"
        ] = self.detector.vertical_motor.get_position()
...
def main():
    ...
    parser.add_option(
        "-a", "--angle_per_frame", default=0.5, type=float, help="Angle per frame [deg]"
    )
    parser.add_option(
