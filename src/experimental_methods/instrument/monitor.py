...
from experimental_methods.instrument.motor import tango_motor, tango_named_positions_motor
from experimental_methods.instrument.camera import camera as redis_camera
...
class monitor(object):  # speech):
    ...
    def can_clear_history(self):
        ...
        return (
            ...
            and self.redis.get(self.clear_flag_key) == "1"
    ...
    def get_observations(self):
        if self.continuous_monitor_name not in [None, "device"]:
            self.observations = self.get_observations_from_history(
...
class eiger_en_out(counter):
    def __init__(self, attribute_name="Ext_Eiger", sleeptime=0.005):
        counter.__init__(self, attribute_name=attribute_name, sleeptime=sleeptime)


class fast_shutter_close(counter):
    def __init__(self, attribute_name="Fast_Shutter_Close", sleeptime=0.05):
        counter.__init__(self, attribute_name=attribute_name, sleeptime=sleeptime)


class fast_shutter_open(counter):
    def __init__(self, attribute_name="Fast_Shutter_Open", sleeptime=0.05):
        counter.__init__(self, attribute_name=attribute_name, sleeptime=sleeptime)


class trigger_eiger_on(counter):
    def __init__(self, attribute_name="Trigger_Eiger_On", sleeptime=0.05):
        counter.__init__(self, attribute_name=attribute_name, sleeptime=sleeptime)


class trigger_eiger_off(counter):
    def __init__(self, attribute_name="Trigger_Eiger_Off", sleeptime=0.05):
        counter.__init__(self, attribute_name=attribute_name, sleeptime=sleeptime)
...
class sai(monitor):
    def __init__(
        ...
        self.keys = [
            [
                "%s_ch%d" % (key, k)
                for key in [
                    self.last_timestamp_key,
                    self.history_timestamp_key,
                    self.history_data_key,
                ]
            ]
    ...
    def run_history(self):
        ...
        while True:
            for channel in range(self.number_of_channels):
                last_timestamp_key, history_timestamp_key, history_data_key = self.keys[
                    channel
                ]

                last_point_data = self.get_historized_channel_values(
                    channel
                )  # about 1ms
                ...
                if history_data != b"0":
                    history_data, new_values = self.merge_two_overlapping_buffers(
                        history_data, last_point_data_buffer
                    )
                    previous_timestamp = float(self.redis.get(last_timestamp_key))
                    history_timestamp = np.frombuffer(
                        self.redis.get(history_timestamp_key)
                    )
                else:
                    history_data, new_values = last_point_data_buffer, len(
                        last_point_data
                    )
                    previous_timestamp = (
                        last_timestamp
                        - new_values * self.get_integration_time() * 1.0e-3
                    )
                    history_timestamp = False

                new_history_timestamps = np.linspace(
                    last_timestamp, previous_timestamp, int(new_values), endpoint=False
                )[::-1]

                history_timestamp = (
                    new_history_timestamps
                    if history_timestamp is False
                    else np.hstack([history_timestamp, new_history_timestamps])
                )
            ...
            if (
                np.any(self.history_sizes > 1.2 * self.history_size_threshold)
                and self.redis.get(self.clear_flag_key) == "1"
                or self.history_sizes.max() > 2 * self.history_size_threshold
            ):
                for channel in range(self.number_of_channels):
                    (
                        last_timestamp_key,
                    ...
                    self.redis.set(
                        history_timestamp_key,
                        history_timestamp[-self.history_size_threshold :],
                    )
                    self.redis.set(
                        history_data_key, history_data[-self.history_size_threshold :]
                    )
    ...
    def get_history(self, start=-np.inf, end=np.inf):
        ...
        for channel in range(self.number_of_channels):
            ...
            mask = np.logical_and(
                channel_timestamps >= start, channel_timestamps <= end
            )
...
class Si_PIN_diode(sai):
    ...
    def get_params(
        self, datafile="/927bis/ccd/gitRepos/flux/xray9507_Si_125um.dat"
    ):  # xray5184.dat
...
class xbpm(monitor):
    ...
    def get_point_and_reference(self):
        point_and_reference = dict(
            [
                (attribute, getattr(self, "get_%s" % attribute)())
...
class peltier(monitor):
    def __init__(self, device_name="i11-ma-c03/op/mono1-pt100.2"):
        monitor.__init__(self)
...
class thermometer(monitor):
    def __init__(self, device_name="i11-ma-cx1/ex/tc.1"):
        monitor.__init__(self)
...
class analyzer(basler_camera):
    def get_point(self):
        return np.array(
            [
                self.get_gaussian_fit_center_x(),
    ...
    def run_history(self):
        ...
        while True:
            ...
            self.data = (
                np.hstack([self.data, last_point_data])
                if len(self.data)
                else last_point_data
            )
            ...
            if self.can_clear_history():
                for item in [self.data, self.timestamps]:
                    self.data = self.data[
                        -self.history_size_threshold * len(last_point_data) :
                    ]
...
class xray_camera(basler_camera):
    def __init__(
        ...
        self.stage_horizontal_observation_position = (
            stage_horizontal_observation_position
        )
    ...
    def insert(self):
        ...
        self.stage_horizontal_motor.set_position(
            self.stage_horizontal_observation_position, wait=True
        )
        self.stage_vertical_motor.set_position(
            self.stage_vertical_observation_position, wait=True
        )
...
class oav_camera(monitor):
    ...
    def get_point(self):
        ...
        com = (
            np.array(center_of_mass(img))
            / np.array(self.cam.get_image_dimensions())[::-1]
        )
...
class tdl_xbpm(monitor):
    ...
    def get_position(self, attributes=["zPos", "xPos"]):
        return dict(
            [
                (attribute, self.device.read_attribute(attribute).value)
...
class tango_monitor(monitor):
    ...
    def wait(self, timeout=30):
        ...
        _start = time.time()
        while (
            self.get_state() not in ["STANDBY", "READY"]
