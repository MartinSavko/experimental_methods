...
class instrument(object):
    ...
    def get_state(self):
        ...
        p[
            "primary_slits_horizotal_position"
        ] = self.primary_slits.get_horizontal_position()
        p[
            "primary_slits_vertical_position"
        ] = self.primary_slits.get_vertical_position()
        ...
        p[
            "secondary_slits_horizotal_position"
        ] = self.secondary_slits.get_horizontal_position()
        p[
            "secondary_slits_vertical_position"
        ] = self.secondary_slits.get_vertical_position()
        ...
        p[
            "experimental_slits_horizontal_gap"
        ] = self.experimental_slits.get_horizontal_gap()
        p[
            "experimental_slits_vertical_gap"
        ] = self.experimental_slits.get_vertical_gap()
        p[
            "experimental_slits_horizotal_position"
        ] = self.experimental_slits.get_horizontal_position()
        p[
            "experimental_slits_vertical_position"
        ] = self.experimental_slits.get_vertical_position()
...
class machine:
    ...
    def get_hu640_currents(self):
        timesteps = [time.time()] + [
            self.hu640.read_attribute("steptimeps%d" % k).value for k in range(1, 4)
        ]
        currents = []
        for k in range(3):
            currents.append(
                [time.time()]
                + [
                    self.hu640.read_attribute("currentps%d" % k).value
                    for k in range(1, 4)
                ]
            )
...
class thermometers:
    ...
    def get_temperatures(self):
        return [
            (
                getattr(self, "tc%d" % k).dev_name(),
                getattr(self, "tc%d" % k).temperature,
            )
            for k in range(1, 12)
        ]
...
class vacuum:
    ...
    def get_pressures(self):
        return [
            (getattr(self, "v%d" % k).dev_name(), getattr(self, "v%d" % k).pressure)
            for k in range(1, 25)
        ]
