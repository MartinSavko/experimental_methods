...
puck2 = {"pan": 11.0, "tilt": -85.8, "zoom": 12700.0}
...
def record(camera, output, duration):
    ...
    if duration is not None:
        while time.time() < start + duration:
            ...
            if camera != "prosilica":
                os.system(
                    "wget http://%s/jpg/image.jpg -O %s/%s_%s.jpg > /dev/null"
                    % (camera, output, camera, str(k).zfill(5))
                )
            else:
...
def record_prosilica(output, duration):
    ...
    if duration is not None:
        while time.time() < start + duration:
            k += 1
            os.system(
                "wget http://%s/jpg/image.jpg -O %s/%s_%s.jpg"
                % (camera, output, camera, str(k).zfill(5))
            )
    else:
        while True:
...
def main():
    ...
    parser.add_option(
        ...
        "--camera",
        type=str,
        default="172.19.10.149",
        help="IP or hostname of camera to record stream from",
    )
    parser.add_option(
        "-d", "--duration", type=float, default=None, help="Duration of recording"
    )
    ...
    parser.add_option(
        ...
        type=str,
        default="",
        help="Camera position and zoom setting optimized for either whole dewar, particular lid (lid1 .. lid3) or puck (puck1 .. puck9) default=%default",
    )
    options, args = parser.parse_args()
    print("options", options)
    print("args", args)
    if options.camera == "cam8" and options.what in [
        "dewar",
        "lid1",
        "lid2",
        "lid3",
