...
DISPLAY_IMAGE = False                           # requires PIL to display image attributes -> requires python 3
SIMULATION = False  # TODO change this flag when there is advanced move time analysis and the server is in simulation
...
class GenericClient(object):
    ...
    def __init__(self):
        self.registeredAttr = {}
        self.monitoredMotors = {}           # dictionary of monitored motors where events will be stored with their timestamps and corresponding motor states
    ...
    def onEventReceived(self, attrName, value, timestamp):
        ...
        if attrName.casefold() == "MotorStates".casefold():
            moving = [x for x in value if x.find("Moving") > 0]

            for m in self.monitoredMotors.keys():
                match = re.match(".*"+m+"=(\w+)", ' '.join(value))
                if match is not None:
                    if len(self.monitoredMotors[m]) == 0 and match.group(1).casefold() == "Moving".casefold():
                        self.monitoredMotors[m].append([timestamp, match.group(1)])
                    if len(self.monitoredMotors[m]) == 1 and (match.group(1).casefold() == "Ready".casefold() or match.group(1).casefold() == "LowLim".casefold() or match.group(1).casefold() == "HighLim".casefold()):
                        # print(match.group(1))
                        with self.moveLock:
                            self.monitoredMotors[m].append([timestamp, match.group(1)])
    ...
    def homeMotor(self, motor, timeout=20):
        ...
        tid = 0
        try:
            maxHomingTime = self.getHomingTimeout(motor)/1000.
        ...
        try:
            ...
            Logger.log('An error occured while homing the motor {} : {}'.format(motor, e), Logger.FAILED)
            if useEvents:
...
#            self.moveLock.release()
            ...
            while self.getMotorState(motor).casefold() != "Initializing".casefold() and time.clock() - start < maxHomingTime:
                pass
        while len(self.monitoredHomingMotors[mName]) == 0 and time.clock() - start < maxHomingTime:
            pass
        if len(self.monitoredHomingMotors[mName]) == 0:
            Logger.log("The %s motor should be initializing, while state is %s." % (motor, self.getMotorState(motor)), Logger.FAILED)
        else:
        ...
        if self.getMotorState(motor) != "Ready":
            Logger.log("The %s motor homing failed, state was %s" % (motor, self.getMotorState(motor)), Logger.FAILED)
        else:
    ...
    def waitAndCheck(self, task_name, id, cmd_start, expected_time, timeout):
        ...
        if act_time >= timeout:
            Logger.log("%s %d failed due to Timeout" % (task_name, id), success=Logger.FAILED)
            return False
        elif act_time >= 3 * expected_time:
            Logger.log("%s %d was a lot longer than expected: %.4f sec " % (task_name, id, act_time),
                       success=Logger.FAILED)
            return True
        else:
            Logger.log("%s %d passed in %.3f sec " % (task_name, id, act_time), success=Logger.OK)
            return True
    ...
    def waitReady(self, timeout):
        ...
        while True:
            ...
            #epics.ca.poll(0.0001, 0.4)      # somehow similar to a time.sleep(0.1)
    ...
    def syncMoveMotor(self, motor, position, timeout=30, use_events=True):
        ...
        if use_events and self.hasEvents():
            ...
            pollingStartClock = time.clock()

        try:
            self[motorPositionAttribute] = position
        except Exception as e:
            Logger.log("The %s motor could not move due to an error %s." % (motor, e), Logger.FAILED)
            if use_events and self.hasEvents():
        ...
        if use_events and self.hasEvents():
            ...
            motorState = self.monitoredMotors[motorName][-1][1]          # read the last motor state thrown in an event
            del self.monitoredMotors[motorName]
            self.moveLock.release()
            self.unsubscribe("MotorStates")
        else:
            ...
            while motorState != "Ready" and motorState != "LowLim" and motorState != "HighLim" :     # check if the motor is ready and has not reached a limit
                motorState = self[motorStateAttribute]
                time.sleep(0.1)
                timeoutExpired = (time.perf_counter() - pollingStartClock) > timeout
                if timeoutExpired:
                    raise Exception("Timeout occurred while moving motor %s to position %.4f" % (motor, position))
                pass
        Logger.trace("Final %s state: %s" % (motor, motorState))

    def moveAndWaitEndOfMove(self, motor, initialPos, position, useAttr, n, useEvents=False, goBack=False, timeout=20, backMove=False):
        """
        ...
        if abs(initialPos - float(self.getMotorPosition(motor))) > 0.01:
            self.moveAndWaitEndOfMove(motor, float(self.getMotorPosition(motor)), initialPos, useAttr, 1, useEvents, False, timeout)
        try:
            expectedMoveTime = max(0.1, float(self.getMotorMoveTime(motor, position)))
            Logger.trace("The %s motor will move from %.4f to %.4f in %.4f s" % (motor, initialPos, position, expectedMoveTime))
        except:
            expectedMoveTime = 0.1
            Logger.trace("The %s motor will move from %.4f to %.4f in unknown time " % (motor, initialPos, position))
        ...
        for i in range(0, n):
            ...
            try:
                ...
                Logger.log("The %s motor could not move due to an error %s. %s" % (motor, e, pass_log), Logger.FAILED)
                if useEvents:
            ...
            if expectedMoveTime > 1:
                ...
                if str(st).casefold() != "Moving".casefold() and len(self.monitoredMotors[mName]) < 2:
                    Logger.log("The %s motor should be moving, while state is %s. %s" % (motor, st, pass_log), Logger.FAILED)
                else:
                    Logger.log("The %s motor movement was triggered. %s" % (motor, pass_log), Logger.OK)

            if useEvents:
                Logger.trace("Waiting for events, timeout = %.4f + %d sec" % (expectedMoveTime, timeout))
                self.moveLock.wait(expectedMoveTime + timeout)
                if len(self.monitoredMotors[mName]) != 2:
                    # timeout occurred
                    realMoveTime = -1
                else:
                    realMoveTime = (self.monitoredMotors[mName][1][0] - self.monitoredMotors[mName][0][0])/1000.
                ...
                while st != "Ready" and st != "LowLim" and st != "HighLim":
                    ...
                    pass
                realMoveTime = time.clock() - start
                state = st
            if realMoveTime == -1:
                log_msg = 'Motor %s: timeout occurred while waiting for end of move with events. %s\n' \
                          % (motor, pass_log)
                Logger.log(log_msg, Logger.FAILED)
            elif ((abs(realMoveTime-expectedMoveTime)-0.5)/expectedMoveTime > 0.2) and not SIMULATION:
                log_msg = "Motor %s: %.4f s elapsed for the movement, " \
                          "%.4f expected (delta : %.4f), percentage : %.4f. %s\n" % \
                          (motor, realMoveTime, expectedMoveTime, (realMoveTime-expectedMoveTime),
                           100*(realMoveTime-expectedMoveTime)/expectedMoveTime, pass_log)
                Logger.log(log_msg, Logger.FAILED)
            elif ((abs(realMoveTime-expectedMoveTime)-0.5)/expectedMoveTime > 0.5) and SIMULATION:
                log_msg = "Motor %s: %.4f s elapsed for the movement, " \
                          "%.4f expected (delta : %.4f), percentage : %.4f. %s.\n" % \
                          (motor, realMoveTime, expectedMoveTime, (realMoveTime-expectedMoveTime),
                           100*(realMoveTime-expectedMoveTime)/expectedMoveTime, pass_log)
                ...
                log_msg = "Motor %s: %.4f s elapsed for the movement, " \
                          ...
                          (motor, realMoveTime, expectedMoveTime, (realMoveTime-expectedMoveTime), pass_log)
                Logger.log(log_msg, Logger.OK)
                Logger.log("Motor %s ending state was %s.\n" % (motor, state), Logger.NA)
            if goBack or i < n-1:
                self.moveAndWaitEndOfMove(motor, position, initialPos, useAttr, 1, useEvents, backMove=True)
    ...
    def isMotorStateConsistant(self, motor):
        try:
            ...
            Logger.log('Error reading {} state, via attribute {} -> {}'.format(motor, att_name, att), Logger.FAILED)
            return
        try:
            meth = self.getMotorState(motor)
        except Exception as e:
            Logger.log('Error reading {} state, via method call.'.format(motor), Logger.FAILED)
            return
        if meth.lower() != att.lower():
            Logger.log('Inconsistency while reading {} state. {} attribute reading : {} \n Method reading : {}.'.format(motor, att_name, meth), Logger.FAILED)
        else:
            Logger.log('Attribute and method state for motor {} are consistant'.format(motor), Logger.OK)

    def isMotorPositionConsistant(self, motor):
        try:
            att = float(self[''.join([motor.replace(" ", ""), "Position"])])
        except Exception as e:
            Logger.log('Error reading {} state, via attribute {}'.format(motor, ''.join([motor.replace(" ", ""), "Position"])), Logger.FAILED)
            return
        try:
            ...
            return
        if (meth - att) > 0.1:
            Logger.log('Inconsistency while reading {} position. {} attribute reading : {} \n Method reading : {}.'.format(motor, ''.join([motor.replace(" ", ""), "Position"]), att, meth), Logger.FAILED)
        else:
            Logger.log('Attribute and method position for motor {} are consistent'.format(motor), Logger.OK)
        ...
        for idx, attr in enumerate(attributes):
            try:
                ...
                if type(attrVal) is list and len(attrVal) >1000:
                    # picture so print it
                    if DISPLAY_IMAGE:
                        Image.open(StringIO(struct.pack('b' * len(attrVal), *attrVal))).show()
                        Logger.log('Reading attribute %s' % (attr.name), Logger.OK)
                else:
                    Logger.log('Reading attribute %s value = %s' % (attr.name, attrVal), Logger.OK)
                if attr.writable is False:
                    tmp = presentRWAttributes.pop(idx+offset)
                    offset -= 1
            except Exception as e:
                Logger.log('Failed to read attribute %s error : %s' % (attr.name, e), Logger.FAILED)
                tmp = presentAttributes.pop(idx+offset)
                tmp = presentRWAttributes.pop(idx+offset)
                offset -= 1
        return presentRWAttributes

    def testPredefinedPositions(self, device, positions, useEvents=lambda self:self.hasEvents(), goBack = False, timeout=30):
        """
    ...
    def setPredefinedPosition(self, device, position, useEvents=False, timeout=30):
        ...
        attr_pos_name = (device + 'Position').replace(' ', '')  # position attributes have no space inside, despite that device names often have spaces
        initial_position = self[attr_pos_name]

        if initial_position is None or initial_position.casefold() == "Unknown".casefold()\
            ...
            try:
                ...
                if useEvents:
                    ...
                    self.moveLock.release()
                Logger.log("Could not move {} to position {} : exception reading {} = {}.".format(device, position, attr_pos_name, e), Logger.FAILED)
            ...
            if useEvents:
                ...
                while self[attr_pos_name] is None or self[attr_pos_name].casefold() != position.casefold():
                    if self[attr_pos_name] is not None:
                        if time.clock()-sTime > timeout:
                            timeoutMsg = " (Timeout)"
                            break
                        else:
                            if useEvents:
            ...
            if not self.isDeviceInPosition(device, position):
                Logger.log("Could not move {} to exact position {} : Found={}.".format(device, position, self[attr_pos_name]), Logger.FAILED)
            else:
                Logger.log('{} device went into position {}.'.format(device, position), Logger.OK)
                return position
        else:
    ...
    def testPhasePositionning(self, prevPos, positions, allPositions=None, testedPhases=None, useEvents=lambda self:self.hasEvents()):
        ...
        if not positions:
            return self.checkTestedAndChangePhase(prevPos, current, allPositions[prevPos], testedPhases, useEvents)
        if current not in positions.keys():
            for pos in positions.keys():
                res = self.checkTestedAndChangePhase(pos, current, allPositions[pos], testedPhases, useEvents)
                if not res:
                    return False
                newDict = dict(positions)
                del newDict[pos]
                if self.testPhasePositionning(current, newDict, allPositions, testedPhases, useEvents):
                    continue
                else:
                    return False
            return self.checkTestedAndChangePhase(prevPos, current, allPositions[prevPos], testedPhases, useEvents)
            ...
            newDict = dict(positions)
            del newDict[current]
            return self.testPhasePositionning(current, newDict, allPositions, testedPhases, useEvents)
        return True

    def checkTestedAndChangePhase(self, position, prevPosition, expectedPos, testedPhases, useEvents=lambda self:self.hasEvents(), timeout = 120):
        if position is None:
            return True
    ...
    def changePhase(self, position, expectedPos, useEvents=False, timeout=120):
        ...
        try:
            tid = self.startSetPhase(position)      # start a task for going to a specific phase
            if tid is None:
                raise Exception("Task for moving to phase %s could not be created." % position)
            th = monitorThread(tid, useEvents, self.monitorTaskInfo)    # monitor the task (to register errors)
            th.start()
        except Exception as e:
            Logger.log('An error occurred while setting the phase {} : {}'.format(position, e), Logger.FAILED)
        ...
        if self['CurrentPhase'] != position:
            ...
            Logger.log('Failed to go to position {}{}.'.format(position, timeoutMsg), Logger.FAILED)
            return False
        else:
            Logger.log('Phase changed to position {} in {:.3f} sec'.format(position, duration), Logger.OK)
        ...
        for key, value in iter(expectedPos.items()):
            ...
            devWaitStartTime = time.clock()
            while not self.isDeviceInPosition(key, value) and time.clock() - devWaitStartTime < 10:
                pass
            if self.isDeviceInPosition(key, value):
                Logger.log('Device {} is in position {}.'.format(key, value), Logger.OK)
            else:
                Logger.log('Device {} is in position {}. {} expected.'.format(key, self[''.join([key, 'Position'])], value), Logger.FAILED)

        self.waitAppReady(time.perf_counter(), 60, 120)
        duration = time.clock() - sTime
        Logger.log('App Ready after Phase changed to position {} in {:.3f} sec'.format(position, duration), Logger.OK)
        return True
    ...
    def toggleIO(self, io, useEvents=lambda self:self.hasEvents(), timeout=10):
        ...
        try:
            ...
            return
        if self[io] == state:
            Logger.log('IO {} cannot be set to {}.'.format(io, not state), Logger.FAILED)
        else:
