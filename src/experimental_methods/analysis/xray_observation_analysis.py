...
def get_observations(r, key):
    ...
    if hasattr(o[0][0], "__len__") and len(o[0][0]) > 1:
        ...
        type(o[0][1]) not in [float, type(None)]
        and hasattr(o[0][1], "__len__")
        and len(o[0][1]) > 1
    ):
...
def get_predictor(timeline, observation, kind="linear", fill_value="extrapolate"):
    predictor = interp1d(
        timeline, observation, kind=kind, fill_value=fill_value, bounds_error=False
    )
...
def plot_single_observation(name_pattern, directory):
    ...
    for sensor in r.keys():
        ...
        for k, f in enumerate(fields[1:]):
            ...
            pylab.plot(
                chronos, fo, color=colors[sensor][k], label="%s %s" % (sensor, f)
            )
...
def main():
    ...
    parser.add_argument(
        "-n",
        "--name_pattern",
        default="en_12650_d_3600_t_100_3",
        type=str,
        help="name pattern",
    )
    parser.add_argument(
        "-d",
        "--directory",
        default="/nfs/data4/2023_Run4/com-proxima2a/2023-09-23/Commissioning",
        type=str,
        help="directory",
    )
        ...
        for sensor in r:
            ...
            for k, f in enumerate(r[sensor]["fields"]):
                ...
                if f not in aligned_results[sensor]:
                    ...
                    aligned_results[sensor][f] = np.hstack(
                        [aligned_results[sensor][f], ok]
                    )
