...
def normalize(fXraw, fYraw, below_edge_boundary, above_edge_boundary):
    below_edge_linear_fit = np.poly1d(
        np.polyfit(fXraw[:below_edge_boundary], fYraw[:below_edge_boundary], 1)
    )

    above_edge_linear_fit = np.poly1d(
        np.polyfit(fXraw[above_edge_boundary:], fYraw[above_edge_boundary:], 1)
    )
...
def get_fYfpp(fXraw, fYnorm, element, below_edge_boundary, above_edge_boundary):
    fYtheory = get_theory_fpp(element, fXraw / 1.0e3)

    below_edge_theory_quadratic_fit = np.poly1d(
        np.polyfit(fXraw[:below_edge_boundary], fYtheory[:below_edge_boundary], 2)
    )

    above_edge_theory_quadratic_fit = np.poly1d(
        np.polyfit(fXraw[above_edge_boundary:], fYtheory[above_edge_boundary:], 2)
    )
...
def singularity(E, E0, get_fpp_intrapolate):
    return -1.0 * get_fpp_intrapolate(E) / (E0 + E)


def get_from_energy_extrapolate_low_to_first(
    energy, element, energy_extrapolate_low, energy_measured_first_point, subintervals
):
    ...
    return (
        integrate(
            integrand_extrapolate,
            energy_extrapolate_low,
            energy_measured_first_point,
            args=(energy, element),
            limit=subintervals,
        )[0]
        * 2.0
...
def get_from_first_to_singularity(
    energy_a, energy_measured_first_point, get_fpp_intrapolate, subintervals
):
    energy, a = energy_a
    ...
    return (
        integrate(
            integrand_intrapolate,
            energy_measured_first_point,
            a,
            args=(energy, get_fpp_intrapolate),
            limit=subintervals,
        )[0]
        * 2.0
        / np.pi
...
def get_singularity_integral(energy_a_b, get_fpp_intrapolate, subintervals):
    ...
    return integrate(
        singularity, a, b, args=(energy, get_fpp_intrapolate), limit=subintervals
    )[0]


def get_from_singularity_to_last(
    energy_b, energy_measured_last_point, get_fpp_intrapolate, subintervals
):
    energy, b = energy_b
...
def get_from_last_to_energy_extrapolate_high(
    energy, element, energy_measured_last_point, energy_extrapolate_high, subintervals
):
...
def calculate_integral(
    ...
    ltf = time.time()
    from_energy_extrapolate_low_to_first = np.apply_along_axis(
        get_from_energy_extrapolate_low_to_first,
        1,
        X[
            :,
            [
    ...
    fts = time.time()
    from_first_to_singularity = np.apply_along_axis(
        get_from_first_to_singularity,
        1,
        X[:, [0, 5]],
        energy_measured_first_point,
        get_fpp_intrapolate,
    ...
    # Singularity
    s = time.time()
    at_singularity = np.apply_along_axis(
        get_singularity_integral, 1, X[:, [0, 5, 6]], get_fpp_intrapolate, subintervals
    )
    at_singularity += -(np.log(np.abs(X[:, 8])) - np.log(np.abs(X[:, 7])))
    at_singularity += -X[:, 2] * (X[:, 6] - X[:, 5])
    at_singularity += -X[:, 3] * (X[:, 8] ** 2 - X[:, 7] ** 2) / 4.0
    at_singularity += -X[:, 4] * (X[:, 8] ** 3 - X[:, 7] ** 3) / 18.0
    at_singularity /= np.pi
    print("singularity took %.3f" % (time.time() - s))

    # From singularity energy+dE up to last data point
    stl = time.time()
    from_sigularity_to_last = np.apply_along_axis(
        get_from_singularity_to_last,
        1,
        X[:, [0, 6]],
        energy_measured_last_point,
        get_fpp_intrapolate,
    ...
    lth = time.time()
    from_last_to_energy_extrapolate_high = np.apply_along_axis(
        get_from_last_to_energy_extrapolate_high,
        1,
        X[
            :,
            [
    ...
    fYfp = (
        from_energy_extrapolate_low_to_first
...
def calculate_integral_for_a_single_point(
    i,
    E0,
    fXraw,
    fElo,
    ...
    # Exrapolate to low energy
    I_low = integrate(
        integrand_extrapolate, fElo, fXraw[0], args=(E0, element), limit=subintervals
    )[0]
    from_fElo_to_first = I_low * 2.0 / np.pi
    ...
    # Singularity
    at_singularity = integrate(
        singularity, a, b, args=(E0, get_fpp_intrapolate), limit=subintervals
    )[0]
    at_singularity += -(np.log(np.abs(d2)) - np.log(np.abs(d1)))
    at_singularity += -fYderiv1[i] * (b - a)
    at_singularity += -fYderiv2[i] * (d2**2 - d1**2) / 4.0
    at_singularity += -fYderiv3[i] * (d2**3 - d1**3) / 18.0
    at_singularity /= np.pi
...
def chooch(sFilename, element, edge):
    ...
    # determine edge
    below_edge_boundary, above_edge_boundary, fEdge = get_edge_boundaries(
        fXraw, fYraw, element
    )
    ...
    fYfpp, fYtheory = get_fYfpp(
        fXraw, fYnorm, element, below_edge_boundary, above_edge_boundary
    )
    # pylab.figure()
    ...
    get_fpp_intrapolate = scipy.interpolate.interp1d(
        fXraw, fYfpps, kind="linear", fill_value="extrapolate"
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        default=None,
        type=str,
        help="Absorption edge entered but will be auto-determined anyway",
    )
    parser.add_option("-r", default=None, type=float, help="Energy resolution")
    parser.add_option(
        "-k", action="store_true", help="Input data will be converted from keV to eV"
    )
    ...
    parser.add_option(
        "-f", default=None, type=str, help="return anom. scattering factors for RemE"
    )
