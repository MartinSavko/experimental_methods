...
__date__ = "04-09-2024"
__copyright__ = "Copyright (c) 2006-2024 Pierre Legrand"
...
def unpack_latticefit2(lattice_string):
    ...
    latt = Lattice((list(map(float, lats[3:9]))), lats[1])
...
def make_xds_image_links(imagename_list, dir_name="img_links",
    ...
    #
    osc_starts = list(collect_im.keys())
    osc_starts.sort()
...
class XDSLogParser:
    ...
    def _get_lattices_table(self):
        ...
        return list(map(unpack_latticefit2, _table))
    ...
    def parse_init(self):
        ...
        rdi["mean_background"] = gpa(" IN A DATA IMAGE PIXEL")
        #
    ...
    def parse_colspot(self):
        ...
        self.lp = self.lp.replace("(INTENSITY<0.1*MEDIAN)","").replace("OMITTED","")
    ...
    def parse_idxref(self):
        ...
        nis, nts = list(map(int, re.match(rexp1, self.lp, re.DOTALL).groups()))
        qx, qy = list(map(float, re.match(rexp2, self.lp, re.DOTALL).groups()))
        meanPixel = (qx+qy)/2
  ...
  Shift in beam position: %(shift_mm)9.2f mm  (%(shift_pixel).1f pixels)\n"""
    ...
    def parse_correct(self):
        ...
        try:
            ...
            K1s, K2s = list(map(float, self.lp[sp1+18: sp2].split()))[:2]
        ...
        rdi.update(dict(list(zip(["ia","ib","ic","ialpha","ibeta","igamma"],
                             rdi["Cell_integrate"]))))
        if self.lp.count("REFINEMENT RESULTS NOT ACCEPTED ") == 0:
            ...
            rdi.update(dict(list(zip(["rvx","rvy","rvz"],rdi["rotation_axis"]))))
            rdi["beam_axis"] = gpa("DIRECT BEAM COORDINATES (REC. ANGSTROEM)")
            rdi.update(dict(list(zip(["bvx","bvy","bvz"],rdi["beam_axis"]))))
        else:
            ...
            rdi.update(dict(list(zip(["rvx","rvy","rvz"],[-1,-1,-1]))))
            rdi["beam_axis"] = [-1, -1, -1]
            rdi.update(dict(list(zip(["bvx","bvy","bvz"],[-1,-1,-1]))))
        ...
        _table = [ list(map(float, l[:26].split()[1:3])) for l in _table ]
...
class XDS:
    ...
    def run_colspot(self):
        ...
        while res.results["spot_number"] < MIN_SPOT_NUMBER and _trial < 4:
            ...
            self.inpParam["SIGNAL_PIXEL"] -= 0.5
            #self.inpParam["SPOT_MAXIMUM_CENTROID"] += 1
            prnt("Insuficiant number of spot (minimum set to %d)." % \
                                                         MIN_SPOT_NUMBER)
            prnt("Trial number %d, recollecting spots with SIGNAL_PIXEL = %.1f" % \
                  (self.inpParam["SIGNAL_PIXEL"], _trial))
            self.run(rsave=True)
            res = XDSLogParser("COLSPOT.LP", run_dir=self.run_dir, verbose=1)
        return res.results
    ...
    def run_pre_correct(self):
	    ...
	    if "XPARM.XDS" in _txt_file[0]:
	       return list(map(float, (_txt_file[3]).split()[1:]))
            else:
	       return list(map(float, (_txt_file[7]).split()[1:]))
...
if __name__ == "__main__":
    ...
    if SIGNAL_PIXEL:
        newPar["SIGNAL_PIXEL"] = SIGNAL_PIXEL
    else:
        newPar["SIGNAL_PIXEL"] = 5.
