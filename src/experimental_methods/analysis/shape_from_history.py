...
def get_raw_projections(
    ...
    for k in range(len(predictions[notion_indices[notion]])):
        (
            ...
            area,
            notion_mask,
        ) = get_notion_prediction(predictions, notion, k=k)
        if present:
...
def get_calibration(master):
    return get_voxel_calibration(
        *cam.get_calibration(zoomposition=np.median(get_zooms(master)))
    )
...
def fit_aspect(
    angles, aspect, aspect_name=None, minimize_method="nelder-mead", debug=False
):
...
def get_analysis(
    predictions, notions, angles, minimize_method="nelder-mead", debug=False
):
    _start = time.time()
    ...
    for notion in notions:
        ...
        analysis[notion_string] = get_notion_analysis(
            predictions, notion, angles, minimize_method=minimize_method, debug=debug
        )

    return analysis


def get_notion_analysis(
    predictions, notion, angles, minimize_method="nelder-mead", debug=False
):
    print("notion", notion)
    notion_string = get_notion_string(notion)
    ...
    for aspect in fits["aspects"]:
        fits["results"][aspect] = fit_aspect(
            valid_angles,
            fits["aspects"][aspect],
    ...
    if debug and notion == "loop":
        for k in range(7):
            ...
            pylab.plot(
                extreme_points_out[i][1],
                extreme_points_out[i][0],
                "o",
                color="red",
                label="extreme_out",
            )
            pylab.plot(
                extreme_points_ini[i][1],
                extreme_points_ini[i][0],
                "o",
                color="magenta",
                label="extreme_ini",
            )
            pylab.plot(
                extreme_points_out_on_axis[i][1],
                extreme_points_out_on_axis[i][0],
                "o",
                color="blue",
                label="extreme_out_on_axis",
            )
            pylab.plot(
                extreme_points_ini_on_axis[i][1],
                extreme_points_ini_on_axis[i][0],
                "o",
                color="green",
                label="extreme_ini_on_axis",
            )
            pylab.legend()
            pylab.show()
    ...
    if debug:
        ...
        for k, aspect in enumerate(fits["aspects"]):
            ...
            model = fits["results"][aspect]["best_model"](
                test_angles, *fits["results"][aspect]["fit"].x
            )
...
def get_volume(
    ...
    vertical_correction = (
        detector_rows / 2
        - notion_analysis["fits"]["results"]["extreme_verticals"]["fit"].x[0]
    )
    ...
    reference_position = get_position_from_vector(
        origin, keys=["CentringX", "CentringY", "AlignmentY", "AlignmentZ"]
    )
    print("reference_position", reference_position)
    # center = np.array([detector_cols/2, detector_rows/2, detector_rows/2])
    center = np.array(
        [reconstruction.shape[0] / 2] + list(np.mean(objectpoints, axis=0)[1:])
    )  # np.array(reconstruction.shape)/2
    print("center", center)
...
def main():
    ...
    parser.add_argument(
        "-H",
        "--history",
        default="/nfs/data2/excenter/2023-04-15T17:22:01.257118/403841_Sat_Apr_15_17:19:14_2023_history.h5",
        type=str,
        help="history",
    )
    ...
    parser.add_argument(
        ...
        "--as_jpegs",
        action="store_true",
        help="query using jpeg strings instead of raw arrays",
    )
    parser.add_argument(
        "-f",
        "--from_scratch",
        action="store_true",
        help="do not use saved predictions, get the new ones",
    )
    parser.add_argument(
        "-n",
        "--notions",
        ...
        help="notions to consider during the analysis",
    )
    parser.add_argument(
        "-m",
        "--minimize_method",
        default="nelder-mead",
        type=str,
        help="scipy.optimize.minimize algorithm (nelder-mead by default)",
    )
    parser.add_argument(
        "-s", "--save", action="store_true", help="save predicitons and analysis resuls"
    )
    ...
    if os.path.isfile(predictions_filename) and not bool(args.from_scratch):
        ...
        _end = time.time()
        print(
            "%d images loaded in %.3f seconds (since start %.4f)"
            % (len(images), _end - _start, _end - _total_start)
        )

        request = {"to_predict": images, "description": args.notions}
    ...
    analysis = get_analysis(
        ...
        args.notions,
        angles,
        minimize_method=args.minimize_method,
        debug=args.debug,
    )

    if args.display:
        notions_aspects = [
            ("foreground", ("extreme_verticals", "extreme_horizontals")),
            ("loop", ("widths", "heights")),
            (
                ["crystal", "loop_inside", "loop"],
                ("widths", "heights", "centroid_verticals", "centroid_horizontals"),
            ),
            (
                "crystal",
                ("widths", "heights", "centroid_verticals", "centroid_horizontals"),
        ...
        for notion, aspects in notions_aspects:
            ...
            for aspect in aspects:
                ...
                pylab.plot(
                    np.rad2deg(test_angles), model, "g-", label="%s fit" % aspect
                )
    ...
    if args.display:
        o3d.visualization.draw_geometries(
            [volume.paint_uniform_color([1.0, 0.706, 0.0])],
            mesh_show_wireframe=True,
            window_name="reconstructed sample volume",
        )
