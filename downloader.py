...
def size_from_h_string(
    size_string, size_dict={"M": 1024**2, "K": 1024, "G": 1024**3, "": 1}
):
...
def download(remote_filename, base_url="http://172.19.10.26/data/", cut_dirs=2):
    try:
        wget_running = subprocess.getoutput(
            'ps aux | grep wget | grep -v "grep --color=auto" | grep %s'
            % remote_filename
        )
    except:
        wget_running = ""
    ...
    if remote_filename not in wget_running:
        wget_line = "(time wget -nH -nv --cut-dirs=%d --backups=3 '%s%s') 2>&1 " % (
            cut_dirs,
            base_url,
            remote_filename,
        )
...
def main():
    ...
    LOGNAME = os.path.join(
        downloader_log_directory, "eiger_DL_logs", "%s.downloader" % username
    )
    WGETLOG = os.path.join(
        downloader_log_directory, "eiger_DL_logs", "%s.WGET" % username
    )
    ...
    with open(WGETLOG + "_%s.log" % (_strdate), "a") as wgetlog:
        while True:
            ...
            for k, filename in enumerate(file_size_and_name):
                log_print(
                    "%d (of %d) filename %s" % (k + 1, to_download, filename),
                    timing=True,
                )
                filesystem_filename = h.unescape(filename).replace(
                    "nfsruche", "nfs/ruche"
                )
                ...
                if rfilename != file_on_disk:
                    log_print(
                        "Unconventional filename rfilename %s, file_on_disk: %s"
                        % (rfilename, file_on_disk),
                        WARNING,
                    )

                try:
                    filesystem_filename_without_potential_uid = filesystem_filename[
                        filesystem_filename.index("/") :
                    ]
                    log_print(
                        "filesystem_filename_without_potential_uid %s"
                        % filesystem_filename_without_potential_uid
                    )
                    potential_uid = filesystem_filename[
                        : filesystem_filename.index("/")
                    ]
                    if potential_uid.isdigit():
                        ...
                        if uid != str(owner_uid):
                            log_print(
                                "File %s belongs to user %s this instance of downloader belongs to user %s, skipping ..."
                                % (filename, uid, owner_uid)
                            )
                            continue
                    else:
                        uid = ""
                        filesystem_filename_without_potential_uid = (
                            "/%s" % filesystem_filename
                        )
                ...
                try:
                    ...
                    if is_master(filesystem_filename_without_potential_uid):
                        command_line = (
                            "/home/experiences/proxima2a/com-proxima2a/bin/fix_master.py -m %s"
                            % filesystem_filename_without_potential_uid
                        )
                    ...
                    destination = os.path.join(
                        default_destination, date.today().isoformat()
                    )
                    check_directory(destination)
                    log_print("destination_default %s" % destination)
                    log_print(
                        "downloading_default %s into %s"
                        % (filesystem_filename_without_potential_uid, destination)
                    )
                    os.chdir(destination)
                    ...
                    log_print(traceback.print_exc())
                    log_print(
                        "Unknown destination for file %s, please inspect"
                        % name_pattern,
                        ERROR,
                    )

                log_print(
                    "Veryfing existence of file %s"
                    % filesystem_filename_without_potential_uid,
                    DEBUG,
                )

                if os.path.exists(filesystem_filename_without_potential_uid):
                    size_on_disk = os.stat(
                        filesystem_filename_without_potential_uid
                    ).st_size
                    size_on_server = size_from_h_string(
                        file_size_and_name[filename]["size"]
                    )
                    ...
                    log_print(
                        "difference in webdav partition size and exact local size %6.4f kB"
                        % (size_webdav_difference / 1024.0,)
                    )
                    log_print(
                        "difference in approximate server and exact local size %6.4f kB"
                        % (size_difference / 1024.0,)
                    )
                    if size_webdav_difference == 0:
                        remove_line = "%s" % filename
                        # remove_line = '%s' % rfilename
                        log_print(
                            "File size for downloaded and original matches perfectly for %s. Removing from server..."
                            % (rfilename,)
                        )
                        ...
                        # remove_line = '%s' % rfilename
                        log_print(
                            "File size match acceptable (%.1f Mb) for %s. Removing from server..."
                            % (size_difference, rfilename)
                        )
                        log_print("removing %s" % remove_line)
                        e.remove_files(remove_line)
                    else:
                        log_print(
                            "File size doesn't match (%.1f kB) for: %s"
                            % (size_difference / 1024.0, rfilename),
                            ERROR,
                        )
                log_print(3 * "\n")
