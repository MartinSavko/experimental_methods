...
def create_mosaic(images):
    ...
    for k, image in enumerate(images):
        ...
        if len(mosaic_shape) == 3:
            mosaic[ro: ro+ims[0], co: co+ims[1]] = image[:]/255.
        else:
            mosaic[ro: ro+ims[0], co: co+ims[1]] = image[:]
    return mosaic 
    
def load_from_file(template):
    images = np.array([img_as_float(imread(img, as_gray=False)) for img in glob.glob(template) if 'report' not in img])
...
def load_from_directory(directory, suffix='.jpg', default_zoom=1):
    ...
    omegas = np.array([float(re.findall('.*omega_([\d\.]*)[_\.].*%s' % suffix, img)[0]) for img in image_names])
    try:
...
def _generate_report(original_images, loop_rectangles, labeled_images, right_loop_boundary, left_loop_boundary, right_pin_boundary, left_pin_boundary, search_line, fits, calibration, gve, destination='', template='optical_path_analysis', display=True):
    ...
    for k, (image, (xmin, ymin, xmax, ymax, centroid, rightmost_point)) in enumerate(zip(original_images, loop_rectangles)):
        c, r = divmod(k, rows)
        ro = r*ims[0]
        co = c*ims[1]
    ...
    ax[4].vlines(right_loop_boundary, normalize(search_line).min(), normalize(search_line).max(), color='k', lw=3, label='right loop boundary')
    ax[4].vlines(left_loop_boundary, normalize(search_line).min(), normalize(search_line).max(), color='k', lw=3, label='left loop boundary')
    ax[4].vlines(right_pin_boundary, normalize(search_line).min(), normalize(search_line).max(), color='y', lw=3, label='right pin boundary')
    ax[4].vlines(left_pin_boundary, normalize(search_line).min(), normalize(search_line).max(), color='y', lw=3, label='left pin boundary')
    ...
    ax[1].plot(np.degrees(angles), np.sqrt(circle_projection_model(angles, *fits['area'][0].x, k=fits['area'][1])), '--', color='green', label='fit area')
    ax[1].plot(np.degrees(angles), widths, 'o', color='blue', label='width')
    ax[1].plot(np.degrees(angles), circle_projection_model(angles, *fits['width'][0].x, k=fits['width'][1]), '--', color='blue', label='fit width')
    ax[1].plot(np.degrees(angles), heights, 'o', color='red', label='height')
    ax[1].plot(np.degrees(angles), circle_projection_model(angles, *fits['height'][0].x, k=k_height), '--', color='red', label='fit height')
    ...
    ax[2].plot(np.degrees(angles), (rightmost_vertical - fits['rightmost_vertical'][0].x[0])*calibration[1], 'o', color='blue', label='rightmost')
    ax[2].plot(np.degrees(angles), (circle_projection_model(angles, *fits['rightmost_vertical'][0].x, k=fits['rightmost_vertical'][1]) - fits['rightmost_vertical'][0].x[0])*calibration[1], '--', color='blue', label='fit rightmost')
    ax[2].set_title('vertical movements')
    ax[2].legend(loc='upper right')
    
    ax[5].plot(np.degrees(angles), (rightmost_horizontal- fits['rightmost_horizontal'][0].x[0])*calibration[0], 'o', color='blue', label='rightmost')
    ax[5].plot(np.degrees(angles), (circle_projection_model(angles, *fits['rightmost_horizontal'][0].x, k=fits['rightmost_horizontal'][1]) - fits['rightmost_horizontal'][0].x[0])*calibration[0], '--', color='blue', label='fit rightmost')
    ...
    print('Report generation took %.2f ' % (report_generation_end - report_generation_start))
...
def segment(omegas, edges, right_loop_boundary, left_loop_boundary, right_pin_boundary, left_pin_boundary, ymax_orig, ymin_orig):
    loop_rectangles = []
    labeled_images = []
    ...
    print('join finished')
    print('Time to segment all %d images using multiprocessing %6.2f' % (len(jobs), end_segmenting_time-start_segmenting_time))
...
def get_difference_images(images, omegas, background_image=None, dark=False, min_size=100, k=5):
    _start = time.time()
    ...
    if dark == False:
        ...
        normalized_background, background_norm = normalize(background_image, return_norm=True)
        difference_images = [subtract_background(img, normalized_background, background_norm) for img in images]
    else:
    ...
    for difference, img, diff, omega in zip(differences, images, difference_images, omegas):
        if difference < k*differences_mean and len(np.argwhere(diff!=0)) > k*min_size:
...
def get_search_lines_peaks(edges, calibration, number_of_views, smoothing_factor=0.050):
    ...
    return peaks_vse, peaks_vve, search_line_hse, search_line_vse, search_line_hve, search_line_vve

def get_pin_and_loop_boundaries(peaks_vse, peaks_vve, search_line_hse, search_line_vse, search_line_hve, search_line_vve, calibration, smoothing_factor=0.05):
    ...
    print('Peak interpretation took %6.2f s ' % (peak_interpret_end - peak_interpret_start))
    ...
    print()
    return right_loop_boundary, left_loop_boundary, right_pin_boundary, left_pin_boundary, search_line, ymax_orig, ymin_orig, gve

def optical_path_analysis(images, omegas, calibration, background_image=None, display=False, smoothing_factor=0.050, min_size=100, threshold_method='otsu', template='optical_alignment', generate_report=False, dark=False, save_images=False, destination='shapes_of_light', save_results=True, generate_color_and_gray=True):
    ...
    edges, images, omegas = get_difference_images(images, omegas, background_image=background_image, dark=dark, min_size=min_size)
    ...
    peaks_vse, peaks_vve, search_line_hse, search_line_vse, search_line_hve, search_line_vve = get_search_lines_peaks(edges, calibration, number_of_views, smoothing_factor=smoothing_factor)
    
    right_loop_boundary, left_loop_boundary, right_pin_boundary, left_pin_boundary, search_line, ymax_orig, ymin_orig, gve = get_pin_and_loop_boundaries(peaks_vse, peaks_vve, search_line_hse, search_line_vse, search_line_hve, search_line_vve, calibration, smoothing_factor=smoothing_factor)
    
    omegas, loop_rectangles, labeled_images = segment(omegas, edges, right_loop_boundary, left_loop_boundary, right_pin_boundary, left_pin_boundary, ymax_orig, ymin_orig)
    ...
    if save_images:
        save_all_images(images, edges, labeled_images, omegas, os.path.basename(template), destination, images_rgb, generate_color_and_gray=generate_color_and_gray)
        
    if generate_report:
        _generate_report(images, loop_rectangles, labeled_images, right_loop_boundary, left_loop_boundary, right_pin_boundary, left_pin_boundary, search_line, fits, calibration, gve, destination=os.path.join(destination, os.path.basename(template)), template='', display=display)
...
def save_all_images(images, edges, labeled_images, omegas, template, destination, images_rgb, generate_color_and_gray=True):
    _s = time.time()
    if generate_color_and_gray:
        ims, prefix, suffix = (images, 'gray', 'jpg')
        save_individual_images(ims, omegas, prefix, os.path.basename(template), suffix, destination)
...
def main():
    ...
    parser.add_option('-i', '--images', type=str, default='/home/smartin/ResearchDevelopmentEducation/learning_dataset/184451_Thu_Oct__3_23:37:20_2019', help='path to the file to the pickled images or one of the images of a series')
    parser.add_option('-d', '--destination', type=str, default='/home/smartin/ResearchDevelopmentEducation/learning_dataset_targets', help='where to save the outputs and intermediates for future learning')
    ...
    parser.add_option('-g', '--generate_report', action='store_true', help='Generate report')
    ...
    name_pattern = options.images.replace('*.png', '').replace('.pck', '').replace('.pickle', '').replace('.h5', '')
    
    if os.path.isdir(os.path.join(options.destination, os.path.basename(name_pattern))) and options.force != True:
    ...
    if 'png' in options.images:
        ...
        if sys.version_info < (3, 0):
            clicks = pickle.load(open(options.images.replace('_history.h5', '_clicks.pickle')))
            calibration = clicks['calibrations'][0][0]
            mean_calibration = np.mean(clicks['calibrations'])
        else:
            clicks = pickle.load(open(options.images.replace('_history.h5', '_clicks.pickle'), 'rb'), encoding='bytes')
            calibration = clicks[b'calibrations'][0][0]
            mean_calibration = np.mean(clicks[b'calibrations'])
        background_image = cam.get_default_background(zoom=cam.get_zoom_from_calibration(mean_calibration)).mean(axis=2)
    ...
    optical_path_analysis(images_rgb, omegas, calibration, background_image=background_image, display=options.display, smoothing_factor=options.smoothing_factor, min_size=options.min_size, threshold_method=options.threshold_method, template=name_pattern, generate_report=generate_report, save_images=save_images, destination=options.destination, save_results=save_results, generate_color_and_gray=generate_color_and_gray)
