...
def main():
    ...
    parser.add_argument('-d', '--destination', default='/nfs/data2/download_issues', type=str, help='Destination')
    ...
    for f in d.get_filenames():
        ...
        if size_difference == 0:
            print('size of the file on local filesystem and the server is the same, removing file from the server')
            d.remove_files(f)
        else:
            print('The size of the file on the local filesystem does not match that on the server, not removing the file please check.')
