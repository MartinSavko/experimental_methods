...
def main():
    ...
    s = time.time()
    image_filename = '%s_zoom_%d_y_%d_x_%d.jpg' % (os.path.join(options.directory, options.name_pattern), int(zoom), int(click[0]), int(click[1]))
    cam.save_image(image_filename)
                   
    double_click_file = h5py.File('%s.h5' % os.path.join(options.directory, options.name_pattern), 'w')
