...
def main():
    ...
    kp = (
        list(itertools.product(kappa_1, phi_1))
        + [(0, 1)]
        + list(itertools.product(kappa_2, phi_2))
        + [(0, 2)]
        + list(itertools.product(kappa_3, phi_3))
        + list(itertools.product(kappa_4, phi_4))
    )
    ...
    line = (
        "optical_alignment.py -d %s -n {K:.0f}_{P:.0f}_{Z:d}_{id:d}_{design:s} --rightmost -K {K:.2f} -P {P:.2f} -z {Z:d} -A -C -R -g 60"
        % directory
    )
    ...
    for kappa, phi in kp:
        ...
        os.system(
            (line + " --save_raw_images").format(
                K=kappa, P=phi, Z=zoom, design="final", id=k
            )
        )
    end = time.time()
    ...
    # end = time.time()
    print(
        "alignment of %d combinations of kappa and phi combinations took %.2f seconds"
        % (k, end - start)
    )
