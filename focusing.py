...
class focusing:
    ...
    def get_mode_2_parameters(self):
        ...
        mode_2_parameters = {
            ...
            # [100.0, 100.0, -175.0, 300.0, 350.0, 400.0, 472.0, 188.0, 140.0, -50.0, -50.0, -200.0]
            "i11-ma-c05/op/mir2-ch.00/targetVoltage": +100.0,
            "i11-ma-c05/op/mir2-ch.01/targetVoltage": +100.0,
            "i11-ma-c05/op/mir2-ch.02/targetVoltage": -175.0,  # -75.0,
            "i11-ma-c05/op/mir2-ch.03/targetVoltage": 300.0,  # +150.0,
            "i11-ma-c05/op/mir2-ch.04/targetVoltage": 350.0,  # +150.0,
            "i11-ma-c05/op/mir2-ch.05/targetVoltage": 400.0,  # +550.0,
            "i11-ma-c05/op/mir2-ch.06/targetVoltage": 472.0,  # +322.0,
            "i11-ma-c05/op/mir2-ch.07/targetVoltage": +188.0,
            "i11-ma-c05/op/mir2-ch.08/targetVoltage": 140.0,  # +40.0,
            "i11-ma-c05/op/mir2-ch.09/targetVoltage": -50.0,  # -100.0,
            "i11-ma-c05/op/mir2-ch.10/targetVoltage": -50.0,  # -100.0,
            ...
            "i11-ma-c06/ex/fent_h.6/position": 0.0,
            "i11-ma-c06/ex/fent_v.6/position": 0.0,
            "i11-ma-c06/ex/shutter-mt_tz/position": 0.0,
            "robot_x": 0.0,
            "robot_y": 0.0,
            "robot_z": 0.0,
        }
    ...
    def get_mode_3_parameters(self):
        mode_3_parameters = {
            ...
            "i11-ma-c04/op/mir.1-mt_rs/position": 0.0,
    ...
    def get_tab2_values(
        ...
        for key in translation:
            tab2_values[key] = [
                "%.5f" % parameters["i11-ma-c05/ex/tab.2/%s" % translation[key]]
            ]
        print(tab2_values)
        return tab2_values
    ...
    def get_current_parameters(self):
        ...
        for mode in self.modes:
            all_parameters += list(
                getattr(self, "get_mode_%s_parameters" % mode)().keys()
            )
    ...
    def write_value(self, ap, value, epsilon=1.0e-2, check_time=0.5, timeout=7):
        _start = time.time()
    ...
    def set_mode(
        ...
        for key, value in parameters_items:
            ...
            if abs(current_value - value) > epsilon:
                print(
                    "key %s, current value %.4f, target value %.4f, will execute ap.write(%s)"
                    % (key, current_value, value, value)
                )
                if interactive:
                    okay = False
        ...
        if adjust_mirror_voltage:
            for mirror_name, mirror in [("VFM", self.mir2), ("HFM", self.mir3)]:
                if interactive:
                    okay = False
                    answer = input(
                        "Should I set target voltages for %s? [Y/n]: " % mirror_name
                    )
                    if answer in ["y", "Y", ""]:
                        okay = True
                if (not interactive or okay == True) and (
                    (mirror_name == "VFM" and modify_vfm_voltages == True)
                    or (mirror_name == "HFM" and modify_hfm_voltages == True)
                ):
        ...
        print(
            "Switch-over from focusing mode %s to focusing mode %s took %.2f seconds"
            % (start_mode, mode, end_set_mode - start_set_mode)
        )
