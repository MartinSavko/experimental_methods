...
def get_images_omegas_calibrations(directory, extension, model_img_size=(256, 320), omega_search_string='color_zoom_[\d]*_([\d\.]*).jpg|.*omega_([\d\.]*)_zoom.*.jpg|color_([\d\.]*).jpg|color_kappa_.*_omega_([\d\.]*).jpg', zoom_search_string='.*_zoom_([\d]*)_.*jpg'):
    _start = time.time()
    import re
    import glob
    ...
    for imagename in imagenames:
        ...
        if angle is not None:
            ...
            to_predict.append(img_to_array(load_img(imagename, target_size=model_img_size), dtype='float32'))
...
def get_reconstruction(projections, angles, detector_rows, detector_cols, detector_row_spacing=1., detector_col_spacing=1.,vertical_correction=0.):
...
def reconstruct(directory='', extension='*.jpg', search_string='color_zoom_[\d]*_([\d\.]*).jpg|.*omega_([\d\.]*)_zoom.*.jpg|color_([\d\.]*).jpg', model_img_size=(256, 320), threshold=0.5, display=False, verbose=False, reconstruction_basename='reconstruction.npy', pcd_basename='reconstruction.pcd', png_basename='reconstruction.png', fits_basename='fits.pickle', predictions_basename='predictions.pickle', projections_basename='projections.npy', notions=['foreground', 'crystal', 'loop', 'stem', 'loop_inside', 'pin', ['crystal', 'loop_inside', 'loop', 'stem']], notion_indices={'crystal': 0, 'loop_inside': 1, 'loop': 2, 'stem': 3, 'pin': 4, 'foreground': 5}, minimize_method='nelder-mead', initial_parameters=[80., 25., np.pi/8], generate_report=False, save_raw_reconstructions=False):
    ...
    to_predict, angles, calibrations, to_predict_imagenames = get_images_omegas_calibrations(directory, extension, model_img_size=model_img_size)
    ...
    print('got %d predictions in %.4f seconds (%.4f seconds per image)' % (len(to_predict), duration, duration/len(to_predict)))
    ...
    for notion in notions:
        ...
        notion_projections_filename = os.path.join(directory, '%s_%s' % (notion_string, projections_basename))
        notion_pcd_filename = os.path.join(directory, '%s_%s' % (notion_string, pcd_basename))
        notion_reconstruction_filename = os.path.join(directory, '%s_%s' % (notion_string, reconstruction_basename))
        notion_png_filename = os.path.join(directory, '%s_%s' % (notion_string, png_basename))
        ...
        for k, angle in enumerate(angles): #range(number_of_projections):
            present, r, c, h, w, r_max, c_max, area, notion_prediction = get_notion_prediction(predictions, notion, k=k, threshold=threshold)
            if not np.isnan(present):
                ...
                projections[:, valid_projection_index, :] = (notion_prediction[:,:]>threshold).astype('float32').T
        ...
        fit_start = time.time()
        fits = {}
        fits['aspects'] = {'heights':  heights, 'widths': widths, 'centroid_verticals': centroid_verticals, 'centroid_horizontals': centroid_horizontals, 'rightmost_point_verticals': rightmost_point_verticals, 'rightmost_point_horizontals': rightmost_point_horizontals, 'areas': areas}
        fits['angles'] = fit_angles
        fits['results'] = {}
        for aspect in fits['aspects']:
            initial_parameters = [np.mean(fits['aspects'][aspect]), np.std(fits['aspects'][aspect])/np.sin(np.pi/4.), np.random.rand()*np.pi]
            fit_circle = minimize(circle_model_residual, initial_parameters, method=minimize_method, args=(fits['angles'], fits['aspects'][aspect]))
            fit_projection = minimize(projection_model_residual, initial_parameters, method=minimize_method, args=(fits['angles'], fits['aspects'][aspect]))
            ...
            fits['results'][aspect] = {'fit_circle': fit_circle, 'fit_projection': fit_projection, 'fit': fit, 'k': k}
        fit_end = time.time()
        ...
        if notion == 'foreground':
            ...
            print('projections shape, center_of_mass', projections.shape, center_of_mass)
            if not np.isnan(center_of_mass[2]):
                vertical_correction =  detector_rows//2 - center_of_mass[2]
            else:
                vertical_correction = 0
        
        reconstruction = get_reconstruction(projections, fit_angles, detector_rows, detector_cols, vertical_correction=vertical_correction)
        if save_raw_reconstructions:
        ...
        for k in range(0, reconstruction.shape[0]):
            reconstruction[k, :, :] = reconstruction[k, :, :] > 0.95*reconstruction[k,:,:].max()
            
        inertia, eigenvalues, eigenvectors, center, Vor, Eor = principal_axes(reconstruction, verbose=verbose)
    ...
    if generate_report:
        ...
        labeled_images = [get_hierarchical_mask_from_predictions(predictions, k=k) for k in range(shape[0])]
        ...
        for k, notion in enumerate(notions):
            ...
            for aspect, color in zip(['heights', 'widths'], ['red', 'green']):
                ...
                l = a.plot(angles_degrees, all_fits[notion_string]['aspects'][aspect], 'o', color=color, label=aspect)
                lines.append(l)
                labels.append(aspect)
                l = a.plot(model_degrees, circle_projection_model(model_angles, *all_fits[notion_string]['results'][aspect]['fit'].x, k=all_fits[notion_string]['results'][aspect]['k']), '--', lw=2, color=color, label='%s_model' % aspect)
                lines.append(l)
            ...
            for aspect, color in zip(['centroid_verticals', 'centroid_horizontals', 'rightmost_point_verticals', 'rightmost_point_horizontals'], ['green', 'blue', 'orange', 'cyan']):
                angles_degrees = np.degrees(all_fits[notion_string]['angles'])
                l = a.plot(angles_degrees, all_fits[notion_string]['aspects'][aspect], 'o', color=color, label=aspect)
                lines.append(l)
                labels.append(aspect)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--directory', default='learning_dataset/100161_Sun_Apr_25_19:15:59_2021', type=str, help='directory')
    parser.add_argument('-D', '--display', action='store_true', help='display')
    parser.add_argument('-f', '--force', action='store_true', help='force')
    parser.add_argument('-g', '--generate_report', action='store_true', help='Generate report')
    parser.add_argument('-R', '--save_raw_reconstructions', action='store_true', help='Save raw reconstructions')
    ...
    if not bool(args.force) and os.path.isfile(report_filename):
        _end = time.time()
        sys.exit('report for %s already present. check took %.4f seconds' % (args.directory, _end-_start))

    reconstruct(directory=os.path.realpath(args.directory), display=bool(args.display), generate_report=bool(args.generate_report), save_raw_reconstructions=bool(args.save_raw_reconstructions))
