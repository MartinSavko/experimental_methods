...
def get_pcd(alignment):
    ...
    if not os.path.isfile(pcd_filename):
        reconstruct(os.path.realpath(alignment), generate_report=True, display=False, save_raw_reconstructions=False)
    pcd = o3d.io.read_point_cloud(pcd_filename)
    return pcd 

def get_parameters(alignment):
    parameters_filename = os.path.join(alignment, '%s_parameters.pickle' % os.path.basename(alignment))
...
def get_better_aligned_ab(points_a, points_b, margin=0.25):
    ...
    print('a_center %s, b_center %s, shift %s, distance %s' % (str(a_center), str(b_center), str(a_center - b_center), str(np.linalg.norm(a_center - b_center, ord=2))))
...
def get_aligned_ab(points_a, points_b, axis=0, margin=0.2):
    ...
    print('original max_points %d, #points difference %d and discrepancy %.4f' % get_difference_and_discrepancy(points_a, points_b))
    ...
    allowed_range = range_s * (1.-margin)
    ...
    ar = points_a[np.logical_and(np.logical_and(points_a[:,0] > a_limit[0], points_a[:,1] > a_limit[1]), points_a[:,2] > a_limit[2])]
    br = points_b[np.logical_and(np.logical_and(points_b[:,0] > b_limit[0], points_b[:,1] > b_limit[1]), points_b[:,2] > b_limit[2])]
    print('after restriction max_points %d, # difference %d and discrepancy %.4f' % get_difference_and_discrepancy(ar, br))
    print()
...
def align(alignment_a, alignment_b, margin=0.25, center=np.array([160, 256, 256]), directions=np.array([1,1,-1]), order=[1,2,0], origin_a=None, origin_b=None, calibration_a=None, calibration_b=None, kappa_a=None, kappa_b=None, phi_a=None, phi_b=None, a_color=[1, 0.706, 0], b_color=[0, 0.706, 1], ab_color=[1, 0, 0.706], cryst_color=[0.706, 0, 1], crystal=None):
    ...
    if kappa_a is None:
        ...
        a_mm = g.get_points_in_goniometer_frame(ao, calibration_a, origin_a[:3], center=center_a, directions=directions, order=order)
        b_mm = g.get_points_in_goniometer_frame(bo, calibration_b, origin_b[:3], center=center_b, directions=directions, order=order)
    else:
    ...
    o3d.visualization.draw_geometries([pcd_a_mm.paint_uniform_color(a_color), pcd_b_mm.paint_uniform_color(b_color), pcd_cryst_mm.paint_uniform_color(cryst_color)], window_name='original')
    ...
    o3d.visualization.draw_geometries([pcd_a_mm.paint_uniform_color(a_color), pcd_b_mm.paint_uniform_color(b_color), pcd_ab_mm.paint_uniform_color(ab_color), pcd_cryst_ab_mm.paint_uniform_color(cryst_color)], window_name='orginal with estimated transform')
    ...
    o3d.visualization.draw_geometries([pcd_b_mm.paint_uniform_color(b_color), pcd_ab_mm.paint_uniform_color(ab_color)], window_name='estimated transform and the real thing')
    
    pcd_at, T_at, center_at, pcd_bt, T_bt, center_bt, pcd_abt, T_abt, center_abt = get_transformed_clouds(pcd_a_mm, pcd_b_mm, pcd_ab_mm)
    
    at = np.asarray(pcd_abt.points)
    bt = np.asarray(pcd_bt.points)
    abt = np.asarray(pcd_abt.points)
    
    o3d.visualization.draw_geometries([pcd_abt.paint_uniform_color(ab_color), pcd_bt.paint_uniform_color(b_color)], window_name='transformed')
    ...
    o3d.visualization.draw_geometries([pcd_att_all.paint_uniform_color(a_color), pcd_btt_all.paint_uniform_color(b_color), pcd_cryst_all.paint_uniform_color(cryst_color)], window_name='aligned, all points')

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('-a', '--alignment_a', default='/nfs/data2/Martin/Research/murko/minikappa_calibration/kappa_95_phi_50_z1', type=str, help='alignment a')
    ...
    parser.add_argument('-b', '--alignment_b', default='/nfs/data2/Martin/Research/murko/minikappa_calibration/kappa_165_phi_100_z1', type=str, help='alignment b')
    ...
    try:
        ...
        print(pcd_a, pcd_b, origin_a, origin_b, calibration_a, calibration_b, kappa_a, kappa_b, phi_a, phi_b)
        pcd_cryst = o3d.io.read_point_cloud(args.crystal)
        align(pcd_a, pcd_b, origin_a=origin_a, origin_b=origin_b, calibration_a=calibration_a, calibration_b=calibration_b, kappa_a=kappa_a, kappa_b=kappa_b, phi_a=phi_a, phi_b=phi_b, crystal=pcd_cryst)
    except:
        traceback.print_exc()
