...
class collect_interface(QDialog, Ui_Collect):
    def __init__(
        self, parent=None, directory="/spool/2017_Run4", name_pattern="collect_$id"
    ):
        ...
        self.exposurePerFrameLineEdit.editingFinished.connect(
            self.exposure_per_frame_effects
        )
        ...
        self.distanceLineEdit.editingFinished.connect(
            self.detector_distance_change_effects
        )
        self.scanSpeedLineEdit.editingFinished.connect(self.scan_speed_change_effects)
        self.framesPerSecondLineEdit.editingFinished.connect(
            self.frames_per_second_change_effects
        )
    ...
    def update_labels(self):
        ...
        self.label_scan_range.setText(
            "scan_range: %s degrees" % self.rangeLineEdit.text()
        )
        self.label_scan_start_angle.setText(
            "scan_start_angle: %s degrees" % self.startLineEdit.text()
        )
        self.label_angle_per_frame.setText(
            "angle_per_frame: %s degrees" % self.slicingLineEdit.text()
        )
        self.label_scan_exposure_time.setText(
            "scan_exposure_time: %s s" % self.exposureLineEdit.text()
        )
        self.label_photon_energy.setText(
            "photon_energy: %s keV" % self.energyLineEdit.text()
        )
        self.label_transmission.setText(
            "transmission: %s " % self.transmissionLineEdit.text()
        )
        self.label_resolution.setText(
            "resolution: %s A" % self.resolutionLineEdit.text()
        )
    ...
    def wavelength_change_effects(self):
        ...
        photon_energy = self.osc.resolution_motor.get_energy_from_wavelength(
    ...
    def detector_distance_change_effects(self):
        ...
        resolution = self.osc.resolution_motor.get_resolution(
            distance=detector_distance, wavelength=wavelength
        )
    ...
    def update_ui(self):
        ...
        frames_per_second = "frames per second: %6.2f" % (
            omega_range / slicing / exposure,
        )
        degrees_per_frame = "degrees per frame: %6.2f " % slicing
        degrees_per_second = "degrees per second: %6.2f" % (omega_range / exposure,)
        detector_distance = (
            "detector distance: %6.2f m"
            % self.osc.resolution_motor.get_distance_from_resolution(
                resolution=resolution
            )
        )  # str(self.osc.detector_distance) #_from_resolution(resolution, energy)
        flux = "flux: %.2e ph/s" % (0.01 * transmission * 1.0e12,)
        exposure_per_frame = "exposure per frame: %6.2f s" % (
            exposure / (omega_range / slicing),
        )
        wavelength = (
            "wavelength: %6.2f A"
            % self.osc.resolution_motor.get_wavelength_from_energy(energy)
        )
        ...
        self.wavelengthLineEdit.setText(
            "%6.2f"
            % (self.osc.resolution_motor.get_wavelength_from_energy(energy) / 1e3,)
        )
        self.fluxLineEdit.setText("%.2e" % (transmission * 0.01 * 1.0e12))
