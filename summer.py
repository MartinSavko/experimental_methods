...
stream_formatter = logging.Formatter(
    "summer.py |%(asctime)s |%(levelname)-7s| %(message)s"
)
    ...
    if minimal:
        ...
        for dataset in required_datasets:
            if type(reference_master[dataset]) == h5py.Dataset:
                ...
                log.info(
                    "attempted creating dataset %s, which is not of the expected type, please check..."
                    % dataset
                )
...
def get_cube(master, images_to_sum, images_per_file=None):
    ...
    for key in datakeys:
        ...
        reblock = block.reshape(
            (new_nimages, images_to_sum, image_height, image_width)
        ).sum(1)
        if cube is not None:
...
def sum_and_save_not_pretending_to_be_smart(
    master,
    images_to_sum,
    images_per_file,
    images_to_loose,
    indices_of_images_to_loose,
    highlimit=-1,
):
    datakeys = list(master["/entry/data"].keys())
    datakeys.sort()
...
def main():
    ...
    parser.add_option(
        ...
        "--master_file",
        type=str,
        default="collect_1_master.h5",
        help="master file",
    )
    parser.add_option(
        "-N", "--new_master_file", type=str, default=None, help="New master filename"
    )
    parser.add_option(
        "-n", "--images_to_sum", type=int, default=None, help="Number of images to sum"
    )
    parser.add_option(
        "-p",
        "--images_per_file",
        type=int,
        default=10,
        help="Number of images per data file. Zero or negative number will result in all of the images being saved in a single data file.",
    )
    parser.add_option(
        "-l",
        "--images_to_loose",
        type=int,
        default=0,
        help="Number of original images not to include in the new images -- useful for simulating increased deadtime or random loss of images",
    )
    parser.add_option(
        "-P",
        "--images_to_process",
        type=int,
        default=-1,
        help="Number of original images to process.",
    )
    parser.add_option(
        "-i",
        "--indices_of_images_to_loose",
        type=str,
        default="-1",
        help='String specifying what images not to include. Depends on the nimages_to_loose value. Either integer, string that will evaluate to python tuple or list or "random" string if images are to be chosen randomly',
    )
    parser.add_option(
        "-f",
        "--foxtrot",
        action="store_true",
        help="Modify resulting master so that it conforms to foxtrot expectations.",
    )
    parser.add_option(
        "-c",
        "--complete_master",
        action="store_true",
        help="Recreate complete master including pixel mask, flatfield and module diagnostics",
    )
    ...
    master_dirname, master_filename = os.path.dirname(
        options.master_file
    ), os.path.basename(options.master_file)
    log.debug(
        "master_dirname, master_filename: %s, %s" % (master_dirname, master_filename)
    )
    if options.new_master_file == None:
        new_name = "%s" % master_filename.replace(
            "_master.h5", "_sum%s_master.h5" % str(images_to_sum)
        )
    else:
    ...
    new_nimages, data_filenames = sum_and_save_not_pretending_to_be_smart(
        new_m,
        images_to_sum,
        images_per_file,
        images_to_loose,
        indices_of_images_to_loose,
        highlimit=options.images_to_process,
    )

    log.debug("new_nimages %s" % new_nimages)
    log.debug("data_filenames %s" % data_filenames)

    new_m["/entry/instrument/detector/detectorSpecific/nimages"].write_direct(
        np.array([new_nimages])
    )

    log.debug(
        "confirm %s" % new_m["/entry/instrument/detector/detectorSpecific/nimages"][()]
    )
    ...
    try:
        for angle in angles_to_modify:
            ...
            if increment != 0:
                ...
                new_m["%s/%s_range_average" % (path, angle)].write_direct(
                    new_range_average
                )
            ...
            new_m.create_dataset(
                "%s/%s_end" % (path, angle), data=new_ends, dtype=dtype
            )
