...
def select_strategy(idxref_results, xds_par, auto=True, LATTICE_GEOMETRIC_FIT_CUTOFF=5):
    ...
    while not valid_inp:
        if not auto:
            selection = raw_input(
                "\n Select the spacegroup [%s, %d]: " % (SPGlib[sel_spgn][1], sel_spgn)
            )
        else:
            selection = "\n"
        _sel = selection.split()
...
class myXDS(XDS):
    ...
    def run_init(self):
        ...
        if BRUTE:
            ...
            bkgr = i1, min(i2, min(i1 + 15, i1 + int(7.0 / dPhi)))
        else:
            bkgr = i1, min(i2, min(i1 + 7, i1 + int(3.0 / dPhi)))
        self.inpParam["BACKGROUND_RANGE"] = bkgr
        self.run(rsave=True)
        res = XDSLogParser("INIT.LP", run_dir=self.run_dir, verbose=1)
        if res.results["mean_background"] < 1.0:
            prnt(
                "INIT has found a very LOW mean background.\n"
                + "  -> Setting FIXED_SCALE_FACTOR for INTEGRATE step.",
                WARNING,
            )
            self.inpParam["DATA_RANGE_FIXED_SCALE_FACTOR"] = i1, i2, 1.0
        return res.results
    ...
    def run_colspot(self):
        ...
        if "slow" in self.mode:
            ...
            frames_per_colspot_sequence = int(round(60.0 / dPhi, 0))
            self.inpParam["VALUE_RANGE_FOR_TRUSTED_DETECTOR_PIXELS"] = 5000, 30000
        ...
        if (max_fn - min_fn + 1) >= _2fpcs:
            ...
            max_frame = min(max_fn, min_fn + int(89.0 / dPhi + _fpcs))
            spot_ranges = (
                (min_fn, min_fn + _fpcs - 1),
        ...
        if BRUTE and len(self.inpParam["SPOT_RANGE"]) == 1:
            self.inpParam["SPOT_RANGE"] = ((min_fn, int(89.0 / dPhi + _fpcs)),)
        ...
        while res.results["spot_number"] < MIN_SPOT_NUMBER and _trial < 4:
            ...
            self.inpParam["SIGNAL_PIXEL"] -= 1.0
            # self.inpParam["SPOT_MAXIMUM_CENTROID"] += 1
            prnt("Insuficiant number of spot (minimum set to %d)." % MIN_SPOT_NUMBER)
    ...
    def run_idxref(
        self, beam_center_search=False, ranking_mode="ZSCORE", beam_center_swap=False
    ):
        ...
        if beam_center_swap:
            ...
            origins = [[y, x], [mx, my], [my, mx], [x, my], [y, mx], [mx, y], [my, x]]
            for origin in origins:
                self.inpParam["ORGX"] = origin[0]
                self.inpParam["ORGY"] = origin[1]
                prnt(
                    "   Testing beam coordinate: (%.2fmm, %.2fmm) = "
        ...
        if beam_center_search:
            RD = res.results
            prnt(
                " Number of possible beam coordinates: %d"
        ...
        if beam_center_search or beam_center_swap:
            ...
            if beam_center_search:
                ...
                best_beam_orig = get_beam_origin(best_beam_coor, best_beam, det_params)
    ...
    def run_integrate(self, image_ranges):
        ...
        if res.results["mean_background_unmasked"] < 1.0:
            ...
            if os.path.isdir(self.run_dir):
                ...
                if self.link_to_images:
                    if not os.path.exists(self.link_name_to_image):
                        os.system(
                            "ln -sf '%s' %s"
                            % (self.collect_dir, self.link_name_to_image)
                        )
    ...
    def run_pre_correct(self):
        ...
        if not is_pointless_installed():
            prnt(
                "Pointless program not installed. XDS will try to guess"
            ...
            try:
                likely_spg, new_cell = pointless(
                    dir_name=self.run_dir, hklinp="INTEGRATE.HKL"
                )
            except:
                raise
    ...
    def run_correct(self, res_cut=(1000, 0), spg_num=0):
        ...
        if spg_num:
            prnt("   ->  Using spacegroup: %s  #%d" % (SPGlib[spg_num][1], spg_num))
        lattice = Lattice(self.inpParam["UNIT_CELL_CONSTANTS"], symmetry=spg_num)
        lattice.idealize()
        self.inpParam["UNIT_CELL_CONSTANTS"] = lattice.cell
        self.inpParam["JOB"] = ("CORRECT",)
...
class myXDSLogParser(XDSLogParser):
    def __init__(
        self, filename="", run_dir="", verbose=False, raiseErrors=True, XDS_PATH=""
    ):
        XDSLogParser.__init__(
            self,
            filename=filename,
            run_dir=run_dir,
            verbose=verbose,
...
if __name__ == "__main__":
    ...
    for o, a in opts:
        ...
        if o in ("-E", "--exec"):
            ...
            if not (os.path.isfile(xdsf) and os.access(xdsf, os.X_OK)):
                print(
                    "WARNING: no 'xds_par' exec found in path %s." % XDS_PATH, WARNING
                )
    ...
    if not _coll.isContinuous(inputf):
        ...
        inputf = make_xds_image_links(
            inputf, os.path.join(newDir, link_dir_name), "image"
        )
    ...
    if newPar["ORGX"] == 0:
        newPar["ORGX"] = newPar["NX"] / 2.0
    if newPar["ORGY"] == 0:
        newPar["ORGY"] = newPar["NY"] / 2.0
    # This is to correct the starting angle in case first image is not 1.
    newPar["STARTING_ANGLE"] = newPar["STARTING_ANGLE"] - newPar[
        "OSCILLATION_RANGE"
    ] * (newPar["DATA_RANGE"][0] - 1)
    ...
    if ORIENTATION_MATRIX:
        try:
            ...
            prnt(
                "Can't import orientation matrix from: %s" % ORIENTATION_MATRIX,
                CRITICAL,
            )
    if WAVELENGTH:
    ...
    prnt(
        "  Selected resolution range:       %.2f - %.2f A"
    ...
    if STEP <= 3:
        ...
        R3 = newrun.run_idxref(
            _beam_center_optimize, _beam_center_ranking, _beam_center_swap
        )
    if R3:
        i = 0
        _selected_cell = []
        ...
        for LAT in R3["lattices_table"]:
            if LAT.fit <= LATTICE_GEOMETRIC_FIT_CUTOFF:
                i += 1
                prnt(fmt_lat % (i, LAT.symmetry_str1, LAT.fit, LAT.multiplicity, LAT))
