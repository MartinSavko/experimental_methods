...
class mare:
    def __init__(
        ...
        self.alpha = np.linspace(-90.0, 90.0, 500)
        self.wavelength = np.linspace(0, 3, 500)

    def mare(self):
        pylab.figure(figsize=(16, 9))
        for hkl in product(
            np.arange(-self.h_max, self.h_max + 1),
            ...
            knew1 = 0.5 * np.dot(np.dot(r, self.ginv), r.T) - np.dot(
                np.dot(r, self.ginv), self.P.T
            )
            self.log.debug("knew1 %s " % knew1)
            knew22 = np.dot(np.dot(r, self.ginv), r.T) - np.dot(
                np.dot(rpnew, self.ginv), rpnew.T
            )
            ...
            if goodRef:
                ...
                y2 = (
                    knew / (np.cos(np.radians(beta)))
                ) ** 2 + p2new / 4  # 1./lambda**2
                y3 = 1.0 / np.sqrt(y2)
