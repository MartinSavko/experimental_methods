...
class goniometer(object):
    ...
    def wait(self, device=None):
        ...
        while green_light is False:
            try:
                if device is None:
                    if self.get_state().lower() in ['moving', 'running', 'unknown']:
                        logging.info("MD2 wait" )
                    elif self.get_status().lower() in ['running', 'unknown', 'setting beamlocation phase', 'setting transfer phase', 'setting centring phase', 'setting data collection phase']:
                        logging.info("MD2 wait" )
                    else:
                        green_light = True
                        return
    ...
    def set_position(self, position, number_of_attempts=3, wait=True):
        print 'goniometer.set_position(), position', position
                
        motor_name_value_list = ['%s=%6.4f' % (motor, position[motor]) for motor in position if position[motor] != None and (motor not in ['beam_x', 'beam_y'])]
    ...
    def get_centringtablevertical_position(self):
        return self.md2.CentringTableVerticalPosition
    
    def get_centringtablefocus_position(self):
        return self.md2.CentringTableFocusPosition
    ...
    def get_align_vector(self, t1, t2, kappa, phi):
        ...
        if c < 0.:
            c = -c
            x = -x
        cos2a = pow(np.dot(self.kappa_axis[direction], align_direction), 2)
