...
RotaXBeamOffset {rotation_axis_offset} # Rotation axis offset 
    ...
    def __init__(
        ...
        self.dose_vs_photon_energy_model = np.poly1d(
            [
                -8.73221788e-05,
                8.59598045e-03,
                -3.60115597e-01,
        ...
        self.model_total_exposure_time = 90.0
    ...
    def get_output_filename_template(self):
        ...
        output_filename_template = os.path.join(
            self.get_output_directory(), filename_template
        )
    ...
    def run(self):
        ...
        line = "java -jar %s -i %s -p %s" % (
            self.get_raddose3d_binary_path(),
            self.get_input_filename(),
            self.get_output_filename_template(),
        )
    ...
    def get_summary_pickle(self):
        if os.path.isfile(self.get_summary_pickle_name()):
            summary_pickle = pickle.load(
                open(self.get_summary_pickle_name(), "rb"), encoding="bytes"
            )
        else:
            self.run()
            summary_pickle = pickle.load(
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        default=30.0,
        type=float,
        help="horizontal size of crystal [default %default µm]",
    )
    parser.add_option(
        "-y",
        "--size_y",
        default=30.0,
        type=float,
        help="vertical size of crystal [default %default µm]",
    )
    parser.add_option(
        "-z",
        "--size_z",
        default=30.0,
        type=float,
        help="depth of crystal [default %default µm]",
    )
    parser.add_option(
        "-a",
        "--unit_cell_a",
        default=79.0,
        type=float,
        help="unit cell a parameter [default %default A]",
    )
    parser.add_option(
        "-b",
        "--unit_cell_b",
        default=79.0,
        type=float,
        help="unit cell b parameter [default %default A]",
    )
    parser.add_option(
        "-c",
        "--unit_cell_c",
        default=38.0,
        type=float,
        help="unit cell c parameter [default %default A]",
    )
    parser.add_option(
        "--unit_cell_alpha",
        default=90.0,
        type=float,
        help="unit cell alpha parameter [default %default°]",
    )
    parser.add_option(
        "--unit_cell_beta",
        default=90.0,
        type=float,
        help="unit cell beta parameter [default %default°]",
    )
    parser.add_option(
        "--unit_cell_gamma",
        default=90.0,
        type=float,
        help="unit cell gamma parameter [default %default°]",
    )
    parser.add_option(
        "-m",
        "--number_of_monomers",
        default=1,
        type=int,
        help="number of monomers [default %default]",
    )
    parser.add_option(
        "-r",
        "--number_of_residues",
        default=129,
        type=int,
        help="number of residues [default %default]",
    )
    parser.add_option(
        "-p",
        ...
        default=0,
        type=float,
        help="heavy elements protein concentration [default %default]",
    )
    parser.add_option(
        "-s",
        ...
        default=0,
        type=float,
        help="heavy elements solvent concentration [default %default]",
    )
    parser.add_option(
        "-f",
        "--solvent_fraction",
        default=0.5,
        type=float,
        help="solvent fraction [default %default]",
    )
    parser.add_option(
        "-F", "--flux", default=1.16e12, type=float, help="flux [default %default]"
    )
    parser.add_option(
        "-X",
        "--beam_size_x",
        default=10.0,
        type=float,
        help="horizontal beam size [default %default µm]",
    )
    parser.add_option(
        "-Y",
        "--beam_size_y",
        default=5.0,
        type=float,
        help="vertical beam size [default %default µm]",
    )
    parser.add_option(
        "-P",
        "--photon_energy",
        default=12.65,
        type=float,
        help="photon energy in keV [default %default keV]",
    )
    parser.add_option(
        "-O",
        "--oscillation_start",
        default=0.0,
        type=float,
        help="oscillation start [default %default°]",
    )
    parser.add_option(
        "-E",
        "--oscillation_end",
        default=360.0,
        type=float,
        help="oscillation end [default %default°]",
    )
    parser.add_option(
        "-T",
        "--total_exposure_time",
        default=90.0,
        type=float,
        help="total exposure time [default %default seconds]",
    )
    parser.add_option(
        "-D",
        "--output_directory",
    ...
    parser.add_option(
        ...
        default="localhost",
        type=str,
        help="where to run the raddose.jar [default %default]",
    )
    parser.add_option(
        "-R",
        "--pixels_per_micron",
        default=1.0,
        type=float,
        help="computational resolution [default %default pixels per micron]",
    )
    parser.add_option(
        "-A",
        "--angular_resolution",
        default=1.0,
        type=float,
        help="angluar resolution [default %default°]",
    )
    parser.add_option(
        "-C",
        "--collimation_type",
        default="Circular",
        type=str,
        help="collimation type, supported values {Rectangular, Circular} [default %default]",
    )
    parser.add_option(
        "--collimation_x",
        default=100.0,
        type=float,
        help="horizontal collimation [default %default µm]",
    )
    parser.add_option(
        "--collimation_y",
        default=100.0,
        type=float,
        help="vertical collimation [default %default µm]",
    )
    parser.add_option(
        "-B",
        "--beam_profile",
        default="Gaussian",
        type=str,
        help="beam profile, supported values {Gaussian, Tophat} [default %default]",
    )
    parser.add_option(
        "--rotation_axis_offset",
        default=0.0,
        type=float,
        help="rotation axis offset [default %default µm]",
    )
    parser.add_option(
        "--raddose3d_binary_path",
        default="/usr/local/bin/raddose3d.jar",
        type=str,
        help="raddose3d.jar location [default %default]",
    )
    parser.add_option(
        "--filename_template",
        default="test_raddose3d",
        type=str,
        help="filename template [default %default]",
    )
