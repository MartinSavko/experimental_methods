...
class independent_edge_slits:
    slits_names = {1: "i11-ma-c02/ex/fent", 2: "i11-ma-c04/ex/fent"}
    ...
    def get_horizontal_position(self):
        return (self.o.get_position() - self.i.get_position()) / 2.0

    def set_horizontal_position(self, position):
        gap = self.get_horizontal_gap()
        i_position = gap / 2.0 - position
        o_position = gap / 2.0 + position
        self.i.wait()
    ...
    def get_vertical_position(self):
        return (self.u.get_position() - self.d.get_position()) / 2.0

    def set_vertical_position(self, position):
        gap = self.get_vertical_gap()
        u_position = gap / 2.0 + position
        d_position = gap / 2.0 - position
        self.u.wait()
