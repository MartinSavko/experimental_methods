...
from goniometer import get_vector_from_position, get_position_from_vector, get_distance, goniometer
...
def check_beam(directory, name_pattern, camera_exposure_time, transmission, photon_energy):
    os.system('beam_align.py -d %s -n %s -e %.3f -t %.1f -p %.3f -m' % (directory, name_pattern, camera_exposure_time, transmission, photon_energy ) )
    g.set_zoom(1, wait=True)

def measure_diffraction_tomography(p1, p2, directory, modifier='diffraction_tomography', resolution=1.28, photon_energy=12650., transmission=5., stepsize=0.015, scan_range=0.01, vertical_range=0.5, angles='[0, 90, 180, 225, 315]', camera_exposure_time=0.035):
    ba_directory=os.path.join(directory, 'beam_align')
    name_pattern='beam_align_before_a' 
    check_beam(ba_directory, name_pattern, camera_exposure_time, transmission, photon_energy)
    ...
    for k, p in enumerate(positions):
        ...
        diftom = diffraction_tomography(directory=tomdir, name_pattern=nampat, scan_start_angles=angles, photon_energy=photon_energy, resolution=resolution, transmission=transmission, vertical_range=vertical_range, analysis=False, conclusion=False, display=False)
        if not os.path.isfile(diftom.get_parameters_filename()):
            diftom.execute()
        os.system('/nfs/data2/Martin/Research/tomography/shape_from_diffraction_tomography.py -d %s -n %s' % (tomdir, nampat))

    name_pattern='beam_align_after_b' 
    check_beam(ba_directory, name_pattern, camera_exposure_time, transmission, photon_energy)
        
def measure_dose_curves(directory, resolution=1.28, photon_energy=12650., transmission=5., shift_criterion=5., acceptable_area_criterion=250., repeats=27, camera_exposure_time=0.035, protocols=['single_point', 'omega_offset', 'helical'], beam_check_delta=15*60.):
    ...
    points_above_criterion_filename = os.path.join(directory, 'points_above_criterion.pickle')

    if os.path.isfile(points_above_criterion_filename):
        points_above_criterion = pickle.load(open(points_above_criterion_filename, 'rb'))
    else:
    ...
    for k in range(repeats):
        ...
        for P, point in enumerate(points_k):        
            ...
            print('point %d (of %d), point ordinal number %d, pass %d (of %d)' % (P+1, no_points, ordinal, k+1, repeats))
            print('chosen protocol is %s' % protocol)
            
            if time.time() - last_check > beam_check_delta:
                print('going to check beam position, %.3f seconds since last check' % (time.time() - last_check))
                checks += 1
            ...
            if protocol == 'single_point':
                directory = os.path.join(dosing_directory, 'point_%d_%s_%.3f' % (ordinal, protocol, omega_offset))
                osc = omega_scan(directory=directory,
                ...
                line_segment = get_distance(p1, p2, keys=['CentringX', 'CentringY', 'AlignmentY', 'AlignmentZ'])
                directory = os.path.join(dosing_directory, 'point_%d_%s_%.3f' % (ordinal, protocol, line_segment))
                osc = helical_scan(directory=directory,
                            name_pattern=name_pattern,
                            scan_range=360.,
                            scan_exposure_time=18,
                            angle_per_frame=0.1,
            ...
            if not os.path.isfile(osc.get_parameters_filename()):
                ...
                for method in ['xds', 'dozor']:
                    os.system('/nfs/data4/2023_Run3/com-proxima2a/2023-06-02/RAW_DATA/Nastya/px2-0007/run_spotfinder.py -d %s -n %s -m %s &' % (directory, name_pattern, method))
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--directory', default='/nfs/data4/2023_Run3/com-proxima2a/2023-06-02/RAW_DATA/Nastya/px2-0007/pos15', type=str, help='directory')
    ...
    parser.add_argument('-t', '--transmission', default=10, type=float, help='transmission')
    parser.add_argument('-v', '--vertical_range', default=0.4, type=float, help='vertical_range')
    parser.add_argument('-o', '--resolution', default=1.28, type=float, help='Resolution [Angstroem]')
    parser.add_argument('-p', '--photon_energy', default=12650., type=float, help='Photon energy ')
    #parser.add_argument('-a', '--angles', default='[0, 90, 180, 225, 315]', type=str, help='tomography angles')
    parser.add_argument('-a', '--angles', default='[0.0, 90.0, 180.0, 225.0, 315.0]', type=str, help='tomography angles')
    ...
    measure_dose_curves(args.directory, resolution=args.resolution, photon_energy=args.photon_energy, transmission=args.transmission, repeats=args.repeats)
    measure_diffraction_tomography(p1, p2, args.directory, modifier='diffraction_tomography_after', resolution=args.resolution, photon_energy=args.photon_energy, vertical_range=args.vertical_range, angles=args.angles, transmission=5)
    measure_diffraction_tomography(p1, p2, args.directory, modifier='diffraction_tomography_after_5deg_oscillation', resolution=args.resolution, photon_energy=args.photon_energy, scan_range=5., vertical_range=args.vertical_range, angles=args.angles, transmission=5)
