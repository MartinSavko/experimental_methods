...
class nested_helical_acquisition(helical_scan):
    ...
    def __init__(
        ...
        self.total_expected_wedges = (
            self.get_nsegments()
        )  # nsegments if nsegments != None else 1

    def get_total_expected_exposure_time(self):
        nsegments = self.get_nsegments()
        return self.scan_exposure_time * (
            1 + (self.overlap * nsegments) / self.scan_range
        )
    ...
    def get_nimages(self):
        nimages = int(
            self.scan_range / self.angle_per_frame / self.get_ntrigger()
        ) + int(float(self.overlap) / self.angle_per_frame)
        return nimages
    ...
    def get_vector_from_position(
        self, position, motors=["AlignmentY", "AlignmentZ", "CentringX", "CentringY"]
    ):
        vector = []
        for motor in motors:
    ...
    def get_position_from_vector(
        self, vector, motors=["AlignmentY", "AlignmentZ", "CentringX", "CentringY"]
    ):
        return dict(list(zip(motors, vector)))

    def get_positions(
        self, motors=["AlignmentY", "AlignmentZ", "CentringX", "CentringY"]
    ):
    ...
    def get_start_angles(self):
        start_angles = (
            np.arange(
                ...
                self.scan_range / self.nsegments,
            )
            - self.overlap / 2.0
        )
        return start_angles
    ...
    def get_wedge_exposure_time(self):
        wedge_exposure_time = (
            float(self.scan_exposure_time) / self.nsegments
            + self.overlap / self.angle_per_frame * self.get_frame_time()
        )
    ...
    def get_orthogonal_segment_from_position(self, position):
        ...
        start["AlignmentZ"] += self.orthogonal_range / 2.0
        end["AlignmentZ"] -= self.orthogonal_range / 2.0
    ...
    def save_parameters(self):
        ...
        if self.snapshot == True:
            ...
            ] = self.camera.md.beampositionvertical
            self.parameters[
                "beam_position_horizontal"
            ] = self.camera.md.beampositionhorizontal
            self.parameters["image"] = self.image
            self.parameters["rgb_image"] = self.rgbimage.reshape(
                (self.image.shape[0], self.image.shape[1], 3)
            )
            scipy.misc.imsave(
                os.path.join(self.directory, "%s_optical_bw.png" % self.name_pattern),
                self.image,
            )
            scipy.misc.imsave(
                os.path.join(self.directory, "%s_optical_rgb.png" % self.name_pattern),
                self.rgbimage.reshape((self.image.shape[0], self.image.shape[1], 3)),
            )
...
def main():
    ...
    parser.add_option(
        "-N", "--nsegments", default=10, type=int, help="Number of segments [int]"
    )
    parser.add_option(
        "-O",
        "--orthogonal_range",
        default=0.05,
        type=float,
        help="Orthogonal range [int]",
    )
    parser.add_option(
        "-V", "--overlap", default=40, type=float, help="Orthogonal range [int]"
    )
    parser.add_option(
        "-B",
        "--position_start",
