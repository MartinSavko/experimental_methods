...
def moments(data, circle, rotate, vheight):
    ...
    height = stats.mode(data.ravel(), keepdims=True)[0][0]
...
def gaussfit(
    data, err=None, params=[], autoderiv=1, return_all=0, circle=0, rotate=1, vheight=1
):
    """
    ...
    if autoderiv == 0:
        ...
        p, cov, infodict, errmsg, success = optimize.leastsq(
            errorfunction, params, full_output=1
        )
    if return_all == 0:
        return p
    elif return_all == 1:
