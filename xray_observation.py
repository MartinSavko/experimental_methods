...
class xray_observation(xray_experiment):
    ...
    specific_parameters_fields = [{'name': 'duration_intention', 'type': 'float', 'description': 'intended duration in seconds'},
                                  {'name': 'fast_shutter_control', 'type': 'bool', 'description': 'whether to attempt to control fast shutter'}]
    
    def __init__(self,
                 name_pattern,
                 directory,
                 duration_intention=60.,
        ...
        super().__init__(name_pattern=name_pattern,
                         directory=directory,
                         photon_energy=photon_energy,
                         transmission=transmission,
                         diagnostic=diagnostic)
        
        self.description = 'X-ray beam observation, Proxima 2A, SOLEIL, %s' % time.ctime(self.timestamp)
        ...
        for name, device_name in [('vfm_pitch', 'i11-ma-c05/op/mir.2-mt_rx'), 
            ...
            so = {'name': name, 'object': tango_motor, 'kwargs': {'device_name': device_name}}
...
def main():
    ...
    parser.add_argument('-n', '--name_pattern', default='xray_observation', type=str, help='name pattern')
    parser.add_argument('-d', '--directory', default='/tmp', type=str, help='directory')
    parser.add_argument('-i', '--duration_intention', default=60., type=float, help='intended observation duration')
    parser.add_argument('-p', '--photon_energy', default=None, type=float, help='photon energy')
    parser.add_argument('-t', '--transmission', default=None, type=float, help='transmission')
    parser.add_argument('-f', '--fast_shutter_control', default=0, type=int, help='fast shutter control')
    args = parser.parse_args()
    print('args', args)
    
    xo = xray_observation(name_pattern=args.name_pattern, directory=args.directory, duration_intention=args.duration_intention, photon_energy=args.photon_energy, transmission=args.transmission, fast_shutter_control=bool(args.fast_shutter_control))
