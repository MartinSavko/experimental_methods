...
def main(halfrange=0.1, nsteps=25, crop=128):
    ...
    for orientation in [0, 180, 270, 90]:
        ...
        for p in x:
            g.set_position(
                {"AlignmentX": p, "Omega": omega_start + orientation}, wait=True
            )
            time.sleep(0.01)
            img = cam.get_image()
            contrast_curve.append(
                cam.get_contrast(
                    image=img[
                        center[0] - crop : center[0] + crop,
                        center[1] - crop : center[1] + crop,
                    ]
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r", "--halfrange", default=0.05, type=float, help="half range"
    )
