...
class undulator_scan(xray_experiment):
    specific_parameter_fields = [
        {
            "name": "start_energy",
            "type": "float",
            "description": "Scan start photon energy in eV",
        },
        {
            "name": "end_energy",
            "type": "float",
            "description": "Scan end photon energy in eV",
        },
        {
            "name": "scan_speed",
            "type": "float",
        ...
        },
        {
            "name": "gap",
            "type": "float",
            "description": "Undulator gap intention in mm",
        },
        {
            "name": "start_wavelength",
            "type": "float",
            "description": "Scan start wavelength in A",
        },
        {
            "name": "end_wavelength",
            "type": "float",
            "description": "Scan end wavelength in A",
        },
        {
            "name": "start_theta",
            "type": "float",
            "description": "Scan start wavelength in rad",
        },
        {
            "name": "end_theta",
            "type": "float",
            "description": "Scan end wavelength in rad",
        },
    ]
    ...
    def get_theta_from_wavelength(
        ...
        theta = (
            np.arcsin((angstrom / units_wavelength) * wavelength / (2 * d))
            / units_theta
        )
        return theta
    ...
    def get_energy_from_theta(
        ...
        wavelength = self.get_wavelength_from_theta(
            theta, units_wavelength=units_wavelength, units_theta=units_theta
        )
        energy = self.get_energy_from_wavelength(
    ...
    def get_theta_from_energy(
        ...
        wavelength = self.get_wavelength_from_energy(
            energy, units_energy=units_energy, units_wavelength=units_wavelength
        )
        theta = self.get_theta_from_wavelength(
            wavelength,
    ...
    def get_wavelength_from_energy(
        self, energy, units_energy=kilo * eV, units_wavelength=angstrom
    ):
        wavelength = h * c / (units_wavelength * units_energy) / energy
        return wavelength

    def get_energy_from_wavelength(
        self, wavelength, units_energy=kilo * eV, units_wavelength=angstrom
    ):
        energy = h * c / (units_wavelength * units_energy) / wavelength
        return energy

    def set_efficient_start_end(self):
        current_energy = self.get_energy_from_theta(
            self.actuator.get_position(),
            units_energy=eV,
            units_theta=degree,
            units_wavelength=angstrom,
        )
        if abs(self.start_energy - current_energy) > abs(
            self.end_energy - current_energy
        ):
    ...
    def prepare(self):
        ...
        self.start_position = self.get_theta_from_energy(
            self.start_energy,
            units_energy=eV,
            units_theta=degree,
            units_wavelength=angstrom,
        )
        self.end_position = self.get_theta_from_energy(
            self.end_energy,
            units_energy=eV,
            units_theta=degree,
            units_wavelength=angstrom,
        ...
        for k in [1, 2, 3, 5, 6]:
            initial_settings.append(
                gevent.spawn(
                    getattr(getattr(self, "slits%d" % k), "set_horizontal_gap"), 4
                )
            )
            initial_settings.append(
                gevent.spawn(
                    getattr(getattr(self, "slits%d" % k), "set_vertical_gap"), 4
                )
            )
            initial_settings.append(
                gevent.spawn(
        ...
        initial_settings.append(
            gevent.spawn(self.undulator.set_position, self.gap, wait=True)
        )
        initial_settings.append(
            gevent.spawn(self.actuator.set_position, self.start_position, wait=True)
        )
        initial_settings.append(
    ...
    def run(self):
        ...
        move = gevent.spawn(
            self.actuator.set_position, self.end_position, timeout=None, wait=True
        )
        move.join()
    ...
    def get_start_wavelength(self):
        return self.get_wavelength_from_energy(
            self.start_energy, units_energy=eV, units_wavelength=angstrom
        )

    def get_end_wavelength(self):
        return self.get_wavelength_from_energy(
            self.end_energy, units_energy=eV, units_wavelength=angstrom
        )

    def get_start_theta(self):
        return self.get_theta_from_energy(
            self.start_energy,
            units_energy=eV,
            units_wavelength=angstrom,
            units_theta=degree,
...
def main():
    ...
    parser.add_option(
        ...
        "--directory",
        type=str,
        default="/tmp/undulator_scan",
        help="Directory to store the results (default=%default)",
    )
    parser.add_option(
        "-n", "--name_pattern", type=str, default="undulator_scan", help="name_pattern"
    )
    ...
    parser.add_option(
        ...
        type=float,
        default=5.0e3,
        help="Lower bound of the energy scan range in eV",
    )
    parser.add_option(
        "-e",
        "--end_energy",
        type=float,
        default=20.0e3,
        help="Upper bound of the energy scan range in eV",
    )
    parser.add_option(
        "-c", "--scan_speed", type=float, default=0.025, help="Scan speed"
    )
    parser.add_option(
        "-o",
        "--optimize",
        action="store_true",
        help="Check current monochromator position and shuffle start, end to speed up the startup",
    )
