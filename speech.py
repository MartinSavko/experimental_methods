...
class speech:
    ...
    def inform(self, force=False):
        if (
            ...
            logging.info(f"length of the last value in bytes {len(self.value)}")
            logging.info(
                f"value_id {self.value_id}, rate {self.get_rate():.2f} Hz (computed over last {self.framerate_window} images)"
            )
            logging.info(f"history images {len(self.history_values)}")
    ...
    def save_history_local(self, filename, start=-np.inf, end=np.inf, last_n=None):
        logging.info(f"saving {self.service_name} {filename}")
        _start = time.time()
        ...
        logging.info(f"save_history_local thread took {time.time() - _start:.4f} seconds")
        
    def _save_history(self, filename, start, end, last_n):
        self.can_clear_history = False
        print("about to read history")
        ...
        duration = self._get_duration(times)
