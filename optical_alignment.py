...
class optical_alignment(experiment):
    ...
    def get_background_image(self):
        ...
        elif os.path.isfile(self.get_background_filename()):
            if '.png' in self.get_background_filename():
                background_image = imread(self.get_background_filename())
            else:
                background_image = pickle.load(open(self.get_background_filename()))
        else:
            alignmentzposition = self.goniometer.get_alignmentz_position()
            self.goniometer.set_position({'AlignmentZ': alignmentzposition + 2})
            backround_imagename, background_image, background_image_id = self.save_optical_image(background=True)
    ...
    def get_images_filename(self):
        return '%s_images.pickle' % self.get_template()
        

    def get_images(self):
        if not hasattr(self, 'images'):
            self.images = pickle.load(open(self.get_images_filename()))
        return [item[2].mean(axis=-1) for item in self.images]
    

    def get_omegas(self):
        if not hasattr(self, 'images'):
            self.images = pickle.load(open(self.get_images_filename()))
        return [item[1] for item in self.images]
    ...
    def save_images(self):
        f = open(self.get_images_filename(), 'w')
