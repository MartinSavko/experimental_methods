...
def get_spots(spots_file):
    try:
        spots = [list(map(float, item.split())) for item in gzip.open(spots_file, 'rb').read().decode(encoding='ascii').split('\n')[:-1]]
    except:
        spots = []
    return spots
...
class image_monitor:
    ...
    def run(self):
        nimages = 0
        while (time.time() - self.start < self.timeout) or (nimages<self.total_expected_images and nimages>=0) :
            ...
            if len(images) > nimages:
                ...
                if self.show_spots:
                    spots_file = os.path.join(os.path.dirname(image_to_load), 'spot_list', os.path.basename(image_to_load).replace('.cbf.gz', '.adx.gz'))
    ...
    def reconnect(self):
        ...
        try:
            ...
            self.adxv.logger.debug('Connected to Host:Port - %s:%i' % (self.host, self.port))
    ...
    def disconnect(self):
        try:
            ...
            self.adxv.logger.debug('DisConnected from Host:Port - %s:%i' % (self.host, self.port))
        except Exception as e:
            self.adxv.logger.debug(e)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-n", "--name_pattern", default='prefix_1', type=str, help='name_pattern')
    parser.add_argument("-d", "--directory", default='/nfs/data2/orphaned_collects', type=str, help='directory')
    parser.add_argument("-t", "--total_expected_images", default=3600, type=int, help='total_expected_images')
    ...
    parser.add_argument("-s", "--dont_show_spots", action='store_false', help="don't show spots")
    args = parser.parse_args()
    print('image monitor args', args)
    print('bool(args.dont_show_spots)', bool(args.dont_show_spots))
    
    im = image_monitor(args.name_pattern, args.directory, total_expected_images=args.total_expected_images, host=args.host, port=args.port, timeout=args.timeout, show_spots=bool(args.dont_show_spots))
    im.run()
