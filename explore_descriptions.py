...
from useful_routines import (
    black, white, yellow, blue, green, magenta, cyan, purpre,
    notions, colors_for_labels,
    movie2images,
    images2movie,
    get_color,
    get_lut,
...
def omalovanka(
    hierarchical_mask, save=False, destination="examples/opti/detections", prefix="hm", suffix="png", lut=None, negative=False,
):
...
def explore(
    ...
    if every:
        ...
        for k, d in enumerate(descriptions):
            ...
            images.append(rgb)
        images2movie(images, movie=os.path.join(destination, f"{prefix}.avi"), codec="mp4v")
    else:
...
def main():
    ...
    parser.add_argument(
        "-D", "--destination", default="examples/opti/detections", type=str, help="destination"
    )
