...
def image_variance(img, window=3):
    wmean, wsqrmean = ( cv2.boxFilter(x, -1, (window, window), borderType=cv2.BORDER_REFLECT) for x in (img, img*img) )
...
def main():
    ...
    parser.add_option('-f', '--filename', default='sum_master.h5', type=str, help='eiger image master file')
    parser.add_option('-t', '--threshold', default=0.3, type=float, help='eiger image master file')
...
for k, p in enumerate(dead):
    if not_truested[p[0], p[1]] != 0:
        continue
    atch =  ax.add_patch(Rectangle(p[::-1] - np.array([0.5, 0.5]), 1, 1, color='red', ec='red', fc='red'))
    if k == 0:
        legend['dead'] = atch
for k, p in enumerate(cold):
    if not_truested[p[0], p[1]] != 0:
        continue
    atch =  ax.add_patch(Rectangle(p[::-1] - np.array([0.5, 0.5]), 1, 1, color='orange', ec='orange', fc='orange'))
    if k == 0:
        legend['cold'] = atch
for k, p in enumerate(hot):
    if not_truested[p[0], p[1]] != 0:
        continue
    atch =  ax.add_patch(Rectangle(p[::-1] - np.array([0.5, 0.5]), 1, 1, color='pink', ec='pink', fc='pink'))
    if k == 0:
        legend['hot'] = atch
for k, p in enumerate(noisy):
    if not_truested[p[0], p[1]] != 0:
        continue
    atch =  ax.add_patch(Rectangle(p[::-1] - np.array([0.5, 0.5]), 1 ,1, color='green', ec='green', fc='green'))
    if k == 0:
        legend['noisy'] = atch
    ...
    new_hott = [tuple(item) for item in list(np.argwhere((img_float - img_medfilt)/(img_float + 1) > options.threshold))]
    
    new_coldd = [tuple(item) for item in list(np.argwhere((img_medfilt - img_float)/(img_float +1) > options.threshold))]
    ...
    for k, p in enumerate(new_cold):
        atch =  ax.add_patch(Rectangle(p[::-1] - np.array([0.5, 0.5]), 1, 1, color='magenta')) #, ec='green', fc='green'))
        if k == 0:
            legend['new_cold'] = atch
    for k, p in enumerate(new_hot):
        atch =  ax.add_patch(Rectangle(p[::-1] - np.array([0.5, 0.5]), 1, 1, color='cyan')) #, ec='green', fc='green'))
        if k == 0:
            legend['new_hot'] = atch
    ...
    for module in modules:
        ...
        print('v_end', v_end)
        print('h_end', h_end)
        ax.add_patch(Rectangle([h_start-0.5, v_start-0.5], mw, mh, alpha=0.5, fc='none', ec='magenta'))
        if module == (5, 0):
            print(module)
        ...
        ax.text(h_start+10, v_start+40, 'Bad pixels in module (%d, %d): %d' % (k, i, submodule[submodule!=0].sum()))
        print('Number of bad pixels in module (%d, %d)' % (k, i), submodule[submodule!=0].sum())
    ...
    plt.legend([legend[key] for key in legend_keys], legend_keys, numpoints=0.1, bbox_to_anchor=(1, 1), fancybox=True, shadow=True)
    plt.show()
