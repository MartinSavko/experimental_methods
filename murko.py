...
def segment_multihead(
    ...
    if include_capillary_images:
        (
            ...
            directory="images_and_labels_capillary", split=0.2
        )
        # val_paths += val_paths_plate
    ...
    for k in range(len(predictions[0])):
        ...
        for notion in notions:
            ...
            description[get_notion_string(notion)] = {
                ...
                "distance_transform": get_distance_transform(notion_mask),
            }
...
def get_distance_transform(mask, distance_type=cv.DIST_L2, mask_size=3):
    dt = cv.distanceTransform(mask, distance_type, mask_size)
    return dt
    ...
    for notion in ["crystal", "loop_inside", "loop"]:
        ...
        if notion_description["present"]:
            if "distance_transform" in notion_description:
                most_likely_click = (
                    get_inner_center(notion_description["distance_transform"]) / shape
                )
            else:
                r = notion_description["r"]
                c = notion_description["c"]
...
def get_extreme_point(
    projection, pa=None, orientation="vertical", extreme_direction=-1
):
    try:
    ...
    for notion in notions:
        ...
        if massage:
            if notion in ["crystal", "loop", "loop_inside", "stem", "pin"]:
                ...
                massager = "filled"
            mask = massage_mask(mask, min_size=min_size, massager="filled")
        if np.any(mask):
...
def plot_analysis(
    ...
    for k, input_image in enumerate(input_images):
        ...
        if image_paths not in [None, []]:
            ...
            template = f"{prefix}_{model_name}"
