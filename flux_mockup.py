...
class flux_mockup:
    def __init__(self, flux_table="./flux_table.pickle", reference_current=500.0):
...
def main():
    ...
    parser.add_argument(
        "-t", "--transmission", type=float, default=1.0, help="transmission <0, 1>"
    )
    parser.add_argument(
        "-m",
        "--machine_current",
    ...
    flux = fm.get_flux(
        machine_current=args.machine_current,
        transmission=args.transmission,
        photon_energy=args.photon_energy,
    )
    print(
        "Proxima 2A beamline estimated flux at sample position for given parameters: %.1f (%.2e) photons per second"
        % (flux, flux)
    )
